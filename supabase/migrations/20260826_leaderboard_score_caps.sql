...
--
-- VERIFIED READ ONLY AGAINST LIVE DATA BEFORE APPLYING, 2026-08-26: old
-- formula against new, over all 3,581 players. Players whose points change: 0.
-- Largest change to any total: 0. Players dropped: 1, QA-Harness-Test.
...
select gc.game, greatest(max(gc.score), 1), 'frozen from live data 2026-08-26'
from public.game_completions gc
where gc.score is not null and gc.score > 0 and gc.game <> 'qa-test'
group by gc.game
on conflict (game) do nothing;

-- Half two: every key the CLIENT can send that has no scores yet, allowlisted
-- with no frozen denominator so it works the day someone finally plays it.
-- This is the half the first draft missed.
insert into public.game_score_caps (game, max_score, note)
values
  ('baseball-connections', null, 'live game, no scores yet 2026-08-26'),
  ('face-off', null, 'live game, no scores yet 2026-08-26'),
  ('football-connect-4', null, 'live game, no scores yet 2026-08-26'),
  ...
  ('hall-of-champions', null, 'live game, no scores yet 2026-08-26'),
  ('idle-arena', null, 'live game, no scores yet 2026-08-26'),
  ('mlb-connect-4', null, 'live game, no scores yet 2026-08-26'),
  ('nba-connect-4', null, 'live game, no scores yet 2026-08-26'),
  ('nfl-connect-4', null, 'live game, no scores yet 2026-08-26'),
  ('nhl-connect-4', null, 'live game, no scores yet 2026-08-26'),
  ('stadium-tycoon', null, 'live game, no scores yet 2026-08-26'),
  ('wonderkid-factory', null, 'live game, no scores yet 2026-08-26')
on conflict (game) do nothing;
