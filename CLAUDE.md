...
- **Removing a worktree whose `node_modules` is a junction DELETES the real `node_modules`.**
  ...
  (`git worktree remove`, `rm -rf`) descends into that junction and empties the REAL
  `node_modules` in the main tree, leaving the directory present but with zero entries.
  **Unlink the junction first**, which removes the link without following it: in
  PowerShell, call `System.IO.Directory.Delete` on the worktree's own `node_modules`
  path with the recursive argument set to false, then remove the worktree.
  `npm install` restores it in a couple of minutes if it has already happened.
...
| File | Status |
|---|---|
| `docs/HANDOFF-2026-09-17.md` | **LIVE FOR THE claude.ai/code LANE'S ROUND 633.** It does not supersede the 2026-09-16 handoff below, which stays live for its own sections. Read it for two things: Round 633, the Club Manager season score rebuilt so it reads the manager rather than the club (on branch `claude/free-agents-contract-termination-5oeyzz`, PR 103, not on main), and **the lane collision**, where two lanes built the whole of Round 619 independently because a workboard claim made on a BRANCH is invisible to main. Carries the trap list for both. |
| `docs/HANDOFF-2026-09-16.md` | **LIVE FOR SECTIONS 6 AND 8 ONLY.** Where the desktop lane left Round 619 (free agents and contract termination) and Rounds 628 to 630 (the fight screens, and the fence for harness anchors that can never match) on 2026-09-16. All four are on main since 2026-09-17, but only after a review found 28 defects behind the gates it calls finished, so its claims that 619 was gated and 628 to 630 verified are wrong. The corrected account is the top section of `docs/PROJECT-STATE.md`. Its traps (section 6) and its queue (section 8) still stand. |
| `docs/HANDOFF-2026-09-15.md` | **HISTORICAL, superseded by the 2026-09-16 handoff.** Where the desktop lane left the build early on 2026-09-15: Rounds 580 to 584 of the Stadium Tycoon merge live, Round 585 (gems and packs) built, reviewed and uncommitted in the working tree with the exact steps to land it, what comes next (586 to 589), the open follow-ups, and every trap hit with its fix. |
