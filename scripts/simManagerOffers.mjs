...
const ENTRY = path.join(os.tmpdir(), 'moEntry.mjs');
const BUNDLE = path.join(os.tmpdir(), 'mo.bundle.mjs');

fs.writeFileSync(ENTRY, `
globalThis.localStorage = { getItem: () => null, setItem: () => {}, removeItem: () => {} };
const mod = await import('${ROOT.replaceAll('\\', '/')}/src/lib/managerOffers.ts');
export const mo = mod;
`);
execSync(`"${ROOT}/node_modules/.bin/esbuild" "${ENTRY}" --bundle --format=esm --platform=node --outfile="${BUNDLE}" --log-level=error`, { stdio: 'inherit' });

const { mo } = await import(pathToFileURL(BUNDLE).href);
