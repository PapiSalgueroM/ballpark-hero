 ...
 *   8. The rollover's field (uclQualifiersFrom) and the whole season two it
 *      builds are byte identical to origin/main's on fixed seeded careers,
 *      including one whose season one was played on origin/main.
 *   9. A season one save written by origin/main loads and plays a whole season
 *      byte identical to origin/main, with its stored group never rewritten.
 ...
 *
 * "origin/main" is a real bundle of the engine at the commit this branch left
 * main from (the merge base, so later commits on main are not mistaken for
 * this branch's): every src file that differs from it is loaded from
 * `git show` instead of from disk. UCL_S1_BASE=<ref> compares against another
 * ref's merge base. Once merged there is nothing left to differ and those
 * sections say so rather than pretending. Math.random and Date.now are
 * pinned for every compared step, and each comparison first proves the engine
 * agrees with itself on the same seed, so a green is not two noisy runs
 * happening to match.
 *
 * NEGATIVE CONTROLS, UCL_S1_CONTROL=<name>. Each rewrites the bundle of THIS
 * branch's engine before it is imported, refuses to run unless its target
 * appears exactly once, and must turn its sections red:
...
const TMP = os.tmpdir();
const OUT = { mine: path.join(TMP, 'uclSeasonOne.bundle.mjs'), base: path.join(TMP, 'uclSeasonOne.base.bundle.mjs'), data: path.join(TMP, 'uclSeasonOne.data.bundle.mjs') };
...
const [mineOut, baseOut, dataOut] = await Promise.all([
  esbuild.build({ ...common, stdin: entry('src/lib/clubManager.ts') }),
  esbuild.build({ ...common, stdin: entry('src/lib/clubManager.ts'), plugins: [fromBase] }),
...
fs.writeFileSync(OUT.base, baseOut.outputFiles[0].text);
...
console.log(`\nBase: ${BASE} as this branch left it (${baseSha.slice(0, 8)}). src files that differ: ${[...changedFromBase].join(', ') || 'none, so every comparison below is trivially equal'}`);
...
/** Same seed, fresh instances, this branch against the base, after proving
 *  this branch agrees with itself. Returns this branch's result and whether
 *  the base matched it. */
const compareEngines = async (label, seed, fn) => {
  const m1 = await fresh('mine');
  const m2 = await fresh('mine');
  const b0 = await fresh('base');
  ...
  const b = pinned(seed, () => json(fn(b0)));
  if (a1 !== b) { fail(`${label}: differs from ${BASE} (${firstDiff(a1, b)})`); return { ok: false, value }; }
...
function ruleFromData(data) {
  ...
  add(data.CM_FINAL_TABLES_2025_26.holders, 'holders');
...
/* ------------------------------------------------------------------ */
await run('6', `A historic era's season one is byte identical to ${BASE}`, async () => {
...
/* ------------------------------------------------------------------ */
await run('7', `A custom club starts outside Europe, byte identical to ${BASE}`, async () => {
  const spec = over => ({
...
/* ------------------------------------------------------------------ */
await run('8', `The rollover's field and season two are byte identical to ${BASE}`, async () => {
  /* Two fixed seeded careers: one whose season one was played on the base
     (the shape every live save has), one on this branch (the new day one). */
  const careers = [
    { label: 'Real Madrid, season one on the base', which: 'base', club: 'Real Madrid', seed: 8001 },
  ...
  return `${fields} rollover fields of 32 and ${twos} whole season twos identical to the base`;
});

/* ------------------------------------------------------------------ */
await run('9', `A season one save from before this round loads and plays unchanged`, async () => {
  /* Chelsea on the base start in Europe by squad tier, which is exactly the
     save a player can have on disk today. It must keep that group. */
  const b0 = await fresh('base');
  const old = pinned(9001, () => b0.startCareer('Chelsea'));
  if (!old.uclGroup) { fail('the base did not put Chelsea in Europe, so this save does not exercise the case'); return ''; }
  if (old.uclField) { fail('the base save already carries a field, so it is not a save from before this round'); return ''; }
  ...
  return loadedOk && playedOk ? `Chelsea's pre Round 612 group (${old.uclGroup.opponents.join(', ')}) loads and plays ${played.week} entries identical to the base` : '';
});
