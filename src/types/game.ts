/* Round 560: the second tiers at the end of this union are here because a
 * relegated club has to go somewhere true. Before this round Footle's club
 * maps kept last season's league, so Girona, Mallorca, Oviedo, Verona, Pisa,
 * Cremonese and Nantes were all still labelled as top flight clubs months
 * after they went down. 'Other' would have been honest but it throws away a
 * fact the site already holds, so the divisions those seven actually play in
 * are named instead. Leagues here are compared by plain equality, there is no
 * country grouping, so adding a value only ever makes a comparison stricter. */
export type League = 'Premier League' | 'La Liga' | 'Serie A' | 'Ligue 1' | 'Bundesliga' | 'Liga Portugal' | 'Eredivisie' | 'Scottish Premiership' | 'Belgian Pro League' | 'Turkish Süper Lig' | 'Austrian Bundesliga' | 'Swiss Super League' | 'Ukrainian Premier League' | 'Russian Premier League' | 'Danish Superliga' | 'Greek Super League' | 'Czech First League' | 'MLS' | 'Saudi Pro League' | 'Brazilian Série A' | 'Argentine Primera División' | 'J1 League' | 'K League 1' | 'Liga MX' | 'A-League' | 'Chinese Super League' | 'Indian Super League' | 'Colombian Primera A' | 'Chilean Primera División' | 'Paraguayan Primera División' | 'Uruguayan Primera División' | 'Peruvian Primera División' | 'Ecuadorian Serie A' | 'Croatian HNL' | 'Serbian SuperLiga' | 'Romanian SuperLiga' | 'Polish Ekstraklasa' | 'Norwegian Eliteserien' | 'Swedish Allsvenskan' | 'Finnish Veikkausliiga' | 'South African Premier Division' | 'Egyptian Premier League' | 'Moroccan Botola' | 'Tunisian Ligue 1' | 'Qatari Stars League' | 'UAE Pro League' | 'Scottish Championship' | 'EFL Championship' | '2. Bundesliga' | 'Segunda División' | 'Serie B' | 'Ligue 2' | 'Other';
