...
/* Round 521: the inbox and the rivalry events, the same engines the
   flagship runs (careerInbox.ts, careerRivalryEvents.ts), bound to the NFL
...
export default function NflMyCareerBoard() {
  ...
  useEffect(() => {
    try {
      ...
      if (restoredPhase !== 'season') markRestoredFinish('nfl-my-career');
  ...
  const handleInboxAnswer = (msgId: string, choiceIdx: number) => {
    if (!career) return;
    const c: CareerState = JSON.parse(JSON.stringify(career));
    const line = answerNflInboxMessage(c, msgId, choiceIdx);
    if (line === null) return;
    setCareer(c);
  ...
  /* Round 521: dismissing a pending rivalry beat applies its effect and
     clears the card, the same {state, log} shape buyNflItem already
     returns. */
  const dismissRivalry = () => {
    if (!career) return;
    const { state, lines } = dismissNflRivalryEvent(career);
    setCareer(state);
  ...
  const unread = nflUnreadInboxCount(career);
