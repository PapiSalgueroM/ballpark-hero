...
import { dismissMlbRivalryEvent } from '@/lib/mlbCareerRivalryEvents';
...
export default function MlbMyCareerBoard() {
  ...
  const handleInboxAnswer = (msgId: string, choiceIdx: number) => {
    if (!career) return;
    const c: MlbCareerState = JSON.parse(JSON.stringify(career));
    const line = answerMlbInboxMessage(c, msgId, choiceIdx);
    if (line === null) return;
    setCareer(c);
  ...
  /* Round 525: dismissing a pending rivalry beat applies its effect and
     clears the card, the same {state, log} shape buyMlbItem already
     returns. */
  const dismissRivalry = () => {
    if (!career) return;
    const { state, lines } = dismissMlbRivalryEvent(career);
    setCareer(state);
  ...
  /* Round 525: a sixth box for the inbox, appended locally rather than
     ...
     NBA and NHL hubs, neither of which has an inbox to show yet, and a box
     with nothing behind it is worse than no box. */
  const unread = mlbUnreadInboxCount(career);
