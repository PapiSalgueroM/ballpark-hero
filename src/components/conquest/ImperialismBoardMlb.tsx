...
import { MLB_TEAMS, MLB_TEAM_MAP, MLB_CONQUEST_MAP } from '@/data/conquestDataMlb';
...
export default function ImperialismBoardMlb() {
  ...
  const [dailyDone, setDailyDone] = useState<ConquestDailyResult | null>(() => loadDailyResult('mlb', todayStr));
  const [dailyStreak, setDailyStreak] = useState(() => loadDailyStreak('mlb', todayStr));
  const [mode, setMode] = useState<'daily' | 'free'>(() => (loadDailyResult('mlb', todayStr) ? 'free' : 'daily'));
  ...
  useEffect(() => {
    ...
    const result: ConquestDailyResult = {
      ...
      team: favorite,
      score,
      empire: mlbStatesOf(owners, favorite).length,
      calls: predictionHits,
      callsTotal: predictionTotal,
      champion,
      championWasYou: champion === favorite,
    };
    const s = saveDailyResult('mlb', result, todayStr);
    setDailyDone(result);
    setDailyStreak(s);
  ...
  const rollPairings = (br: BracketState | null) => {
    if (br) {
      ...
      setPairings(mlbRandomPairings(rngRef.current));
    }
    setPrediction(null);
  };

  const start = (teamId: string) => {
    rngRef.current = mode === 'daily' ? dailyConquestRng('mlb', todayStr) : Math.random;
  ...
  const playRound = () => {
    ...
    for (const [h, a] of pairings) {
      games.push(resolveMlbGame(h, a, next, rngRef.current));
    }
        ...
        {playedToday ? (
          <div className="rounded-2xl border border-gold/40 bg-card p-5 text-center animate-in fade-in zoom-in-95 duration-300">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              You rode {mlbTeamLabel(dailyDone.team)} to {dailyDone.empire} territor{dailyDone.empire === 1 ? 'y' : 'ies'} and called {dailyDone.calls}/{dailyDone.callsTotal} games.
              {dailyDone.championWasYou ? ' Your empire took the whole map.' : ` ${mlbTeamLabel(dailyDone.champion)} took the map.`}
            </p>
            ...
            <div className="mt-4 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              ...
              <ShareButtons
                ...
                customText={dailyShareText('MLB Imperialism', '/conquest-mlb', dailyDone, dailyStreak, mlbTeamLabel(dailyDone.champion), mlbTeamLabel(dailyDone.team))}
              />
            </div>
  ...
  return (
    <div className="space-y-4">
      ...
      {/* map */}
      <ConquestRegionMap sport={MLB_CONQUEST_MAP} owners={owners} battle={battleView} takeover={takeover} />
          ...
          <div className="mt-4 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            <ShareButtons
              ...
              customText={mode === 'daily' && dailyDone
                ? dailyShareText('MLB Imperialism', '/conquest-mlb', dailyDone, dailyStreak, mlbTeamLabel(champion), favorite ? mlbTeamLabel(favorite) : 'team')
                : `MLB Imperialism 🗺️ ${mlbTeamLabel(champion)} took the whole map. My ${favorite ? mlbTeamLabel(favorite) : 'team'} finished with ${myStates} territories and I called ${predictionHits}/${predictionTotal} games. Score ${score}. douknowball.com/conquest-mlb`}
            />
          </div>
        </div>
      )}
