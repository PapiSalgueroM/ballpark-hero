// MLB Conquest map (2026-08-05). Derived from ConquestMapNba.tsx, pointed
// at MLB_TEAM_MAP. It reuses the NBA territory geometry (NBA_STATES + the
// shared helpers) because those 58 rendered shapes are pure US geography
// with no sport-specific content, and the MLB seed map assigns exactly
// that territory set.

import { NBA_STATES } from '@/data/usStatesPaths';
import { isLightColor } from '@/data/conquestData';
import { MLB_TEAM_MAP } from '@/data/conquestDataMlb';
...
export default function ConquestMapMlb({
  ...
  const getColor = (stateId: string) => {
    ...
    return MLB_TEAM_MAP.get(teamId)?.color || '#2a3040';
  };
  ...
  const stateLabels = useMemo(() => {
    return teamBlobs.map(blob => {
      ...
      const color = MLB_TEAM_MAP.get(teamId)?.color || '#4a4a4a';
  ...
  const hoveredTeam = hoveredTeamId ? MLB_TEAM_MAP.get(hoveredTeamId) : null;
  ...
  const attackerColor = attackingTeam ? (MLB_TEAM_MAP.get(attackingTeam)?.color || '#ffffff') : '#ffffff';
    ...
    <div className="relative w-full">
      ...
      >
        ...
        {NBA_STATES.map(state => {
          ...
          if (teamId) {
            const teamColor = MLB_TEAM_MAP.get(teamId)?.color || '#2a3040';
