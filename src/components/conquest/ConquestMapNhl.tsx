// NHL Conquest map (2026-08-05). Derived from ConquestMapNba.tsx, pointed
// at NHL_TEAM_MAP. It reuses the NBA territory geometry (NBA_STATES + the
// shared helpers) because those 58 rendered shapes are pure US geography
// with no sport-specific content, and the NHL seed map assigns exactly
// that territory set.
...
export default function ConquestMapNhl({
  ...
  const getColor = (stateId: string) => {
    ...
    return NHL_TEAM_MAP.get(teamId)?.color || '#2a3040';
  };
  ...
  const stateLabels = useMemo(() => {
    return teamBlobs.map(blob => {
      ...
      const color = NHL_TEAM_MAP.get(teamId)?.color || '#4a4a4a';
  ...
  const hoveredTeam = hoveredTeamId ? NHL_TEAM_MAP.get(hoveredTeamId) : null;
  ...
  const attackerColor = attackingTeam ? (NHL_TEAM_MAP.get(attackingTeam)?.color || '#ffffff') : '#ffffff';
    ...
    <div className="relative w-full">
      ...
      >
        ...
        {NBA_STATES.map(state => {
          ...
          if (teamId) {
            const teamColor = NHL_TEAM_MAP.get(teamId)?.color || '#2a3040';
