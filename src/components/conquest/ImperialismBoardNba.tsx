...
import {
  ...
  seedNbaEmpires, nbaRandomPairings, resolveNbaGame, nbaBuildHeadlines,
  nbaEmpireCounts, nbaLandlessTeams, nbaStatesOf, nbaPlayoffSeeds, nbaTotalConquest,
  nbaHomeWinProb, nbaTeamLabel, nbaFinalScore,
  nbaEmptyRecords, nbaApplyRecords, nbaRecordLabel,
  type NbaImpGame, type NbaImpRoundResult, type NbaImpRecords,
} from '@/lib/imperialismNba';
...
export default function ImperialismBoardNba() {
  ...
  const [lastRound, setLastRound] = useState<NbaImpRoundResult | null>(null);
  const [champion, setChampion] = useState<string | null>(null);
  const [madePlayoffs, setMadePlayoffs] = useState(false);
  const [records, setRecords] = useState<NbaImpRecords>({});
  ...
  const [dailyDone, setDailyDone] = useState<ConquestDailyResult | null>(() => loadDailyResult('nba', todayStr));
  const [dailyStreak, setDailyStreak] = useState(() => loadDailyStreak('nba', todayStr));
  const [mode, setMode] = useState<'daily' | 'free'>(() => (loadDailyResult('nba', todayStr) ? 'free' : 'daily'));
  const rngRef = useRef<() => number>(Math.random);
  const dailySaved = useRef(false);

  const counts = useMemo(() => nbaEmpireCounts(owners), [owners]);
  const total = Object.keys(owners).length;
  const landless = useMemo(() => nbaLandlessTeams(owners), [owners]);
  ...
  useEffect(() => {
    ...
    const result: ConquestDailyResult = {
      ...
      team: favorite,
      score,
      empire: nbaStatesOf(owners, favorite).length,
      calls: predictionHits,
      callsTotal: predictionTotal,
      champion,
      championWasYou: champion === favorite,
    };
    const s = saveDailyResult('nba', result, todayStr);
    setDailyDone(result);
    setDailyStreak(s);
  ...
  const rollPairings = (br: BracketState | null) => {
    if (br) {
      ...
      setPairings(nbaRandomPairings(rngRef.current));
    }
    setPrediction(null);
  };

  const start = (teamId: string) => {
    rngRef.current = mode === 'daily' ? dailyConquestRng('nba', todayStr) : Math.random;
    dailySaved.current = false;
    setFavorite(teamId);
    setOwners(seedNbaEmpires());
    setRound(1);
    setBracket(null);
    setChampion(null);
  ...
  const playRound = () => {
    const next = { ...owners };
    const games: NbaImpGame[] = [];
    for (const [h, a] of pairings) {
      games.push(resolveNbaGame(h, a, next, rngRef.current));
    }
    const nextRecords = nbaApplyRecords(records, games);
    ...
    const label = inPlayoffs ? NBA_PLAYOFF_LABELS[bracket!.round] : `Round ${round}`;
    setLastRound({ round, label, games, headlines: nbaBuildHeadlines(games, next, nextRecords) });
    setOwners(next);
    ...
    const wiped = nbaTotalConquest(next);
    if (wiped) {
      setChampion(wiped);
      setPhase('recap');
      return;
    ...
    if (round >= NBA_REGULAR_ROUNDS) {
      const seeds = nbaPlayoffSeeds(next, nextRecords);
    ...
    return (
      <div className="space-y-4">
        ...
        {playedToday ? (
          <div className="rounded-2xl border border-gold/40 bg-card p-5 text-center animate-in fade-in zoom-in-95 duration-300">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              You rode {nbaTeamLabel(dailyDone.team)} to {dailyDone.empire} territor{dailyDone.empire === 1 ? 'y' : 'ies'} and called {dailyDone.calls}/{dailyDone.callsTotal} games.
              {dailyDone.championWasYou ? ' Your empire took the whole map.' : ` ${nbaTeamLabel(dailyDone.champion)} took the map.`}
            </p>
            ...
            <div className="mt-4 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
              ...
              <ShareButtons
                gameName="NBA Imperialism"
                gamePath="/conquest-nba"
                score={`${dailyDone.score} pts`}
                customText={dailyShareText('NBA Imperialism', '/conquest-nba', dailyDone, dailyStreak, nbaTeamLabel(dailyDone.champion), nbaTeamLabel(dailyDone.team))}
              />
            </div>
        ...
        <div className="rounded-2xl border border-border bg-card p-4 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            The map starts as a true imperialism split: every territory belongs to its nearest NBA
            arena. Every round, winners conquer the loser's ENTIRE empire. Wiped-out teams keep
            playing, and one win takes it all back. Ride your team to the end, call their games, and pray.
          </p>
          {mode === 'daily' && (
  ...
  const myStates = favorite ? nbaStatesOf(owners, favorite).length : 0;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
        <span className="rounded-full border border-border bg-card px-3 py-1 font-bold text-foreground">
          {inPlayoffs ? NBA_PLAYOFF_LABELS[bracket!.round] : `Round ${round}/${NBA_REGULAR_ROUNDS}`}
        </span>
      ...
      {landless.length > 0 && (
        <p className="text-center text-[11px] text-muted-foreground">
          🏴 Wiped out but still dangerous: {landless.map(nbaTeamLabel).join(', ')}
        </p>
      )}
              ...
              .map((tid, i) => (
                ...
                >
                  <span className={cn('truncate', i < 8 ? 'font-semibold text-foreground' : 'text-muted-foreground')}>
                    {i + 1}. {nbaTeamLabel(tid)}
                  </span>
                  <span className="ml-2 shrink-0 text-muted-foreground">
                    {counts.get(tid)} tr · {nbaRecordLabel(records[tid])}
        ...
        <div className="rounded-2xl border border-gold/40 bg-card p-4 text-center">
          ...
          <div className="mt-3 flex items-center justify-center gap-2">
            {[featured[0], featured[1]].map((tid, i) => {
              const t = NBA_TEAM_MAP.get(tid)!;
              const p = i === 0 ? nbaHomeWinProb(featured[0], featured[1]) : 1 - nbaHomeWinProb(featured[0], featured[1]);
              return (
                <button
                  key={tid}
                ...
                >
                  ...
                  <span className="block text-[10px] text-muted-foreground">
                    {counts.get(tid)} territor{counts.get(tid) === 1 ? 'y' : 'ies'} · {nbaRecordLabel(records[tid])} · {Math.round(p * 100)}% to win
                  </span>
                </button>
              );
            })}
          ...
          >
            <Flag className="h-4 w-4" /> Play {inPlayoffs ? NBA_PLAYOFF_LABELS[bracket!.round] : `Round ${round}`}
          </button>
      ...
      {phase === 'done' && champion && (
        <div className="rounded-2xl border border-gold/50 bg-card p-5 text-center">
          ...
          <p className="mt-2 font-display text-2xl font-black text-foreground">
            {nbaTeamLabel(champion)} rule the map
          </p>
          <p className="mt-1 text-sm text-muted-foreground">
            {champion === favorite
              ? 'Your empire. Your dynasty. Absolute scenes.'
              : favorite && myStates > 0
                ? `Your ${nbaTeamLabel(favorite)} held ${myStates} territor${myStates === 1 ? 'y' : 'ies'} to the end.`
          ...
          <div className="mt-4 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            ...
            <ShareButtons
              ...
              customText={mode === 'daily' && dailyDone
                ? dailyShareText('NBA Imperialism', '/conquest-nba', dailyDone, dailyStreak, nbaTeamLabel(champion), favorite ? nbaTeamLabel(favorite) : 'team')
                : `NBA Imperialism 🗺️ ${nbaTeamLabel(champion)} took the whole map. My ${favorite ? nbaTeamLabel(favorite) : 'team'} finished with ${myStates} territories and I called ${predictionHits}/${predictionTotal} games. Score ${score}. douknowball.com/conquest-nba`}
            />
          </div>
        </div>
      )}
