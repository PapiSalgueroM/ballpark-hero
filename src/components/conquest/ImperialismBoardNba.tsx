...
export default function ImperialismBoardNba() {
  ...
  const [dailyDone, setDailyDone] = useState<ConquestDailyResult | null>(() => loadDailyResult('nba'));
  const [dailyStreak, setDailyStreak] = useState(() => loadDailyStreak('nba'));
  const [mode, setMode] = useState<'daily' | 'free'>(() => (loadDailyResult('nba') ? 'free' : 'daily'));
  ...
  useEffect(() => {
    ...
    const s = saveDailyResult('nba', result);
    setDailyDone(result);
    setDailyStreak(s);
  ...
  const start = (teamId: string) => {
    rngRef.current = mode === 'daily' ? dailyConquestRng('nba') : Math.random;
