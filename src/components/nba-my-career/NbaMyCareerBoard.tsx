...
/* Round 470: the money app, the gram, the rival card and the badges, on the
   ...
   careerSocial, careerBadges), bound to basketball in nbaCareerMoney.ts and
   nbaCareerLoop.ts. */
...
import { dismissNbaRivalryEvent } from '@/lib/nbaCareerRivalryEvents';
...
export default function NbaMyCareerBoard() {
  ...
  useEffect(() => {
    try {
      ...
      setCareer(repairNetWorth(s.c, id => getNbaSpendItem(id)?.cost ?? 0));
  ...
  const create = () => {
    ...
    setDraftDay({ pick: c.draftPick, teamLabel: nbaTeamLabelOf(c.team), playerName: c.name, lines: [pressureLine, roleNote], firstRoundEnd: FIRST_ROUND_END });
    setPhase('season');
    persist(c, 'season', tq);
  };
  ...
  const playSeason = () => {
    ...
    if ((c.suspendedSeasons ?? 0) > 0) {
      ...
      c.headlines = pushHeadlines(c.headlines, nbaHeadlinesFor(c, banned));
    ...
    c.headlines = pushHeadlines(c.headlines, nbaHeadlinesFor(c, line));
    setLastLine(line);
  ...
  const handleMoney = (action: MoneyAction) => {
    if (!career) return;
    const c: NbaCareerState = JSON.parse(JSON.stringify(career));
    const res = nbaMoneyAct(c, action);
    if (!res.ok) return;
    setCareer(c);
  ...
  /* Round 525: the inbox's one write path, the same shape the flagship's
     phone and the NFL board use. Refuses rather than throws when the
  ...
  const handleInboxAnswer = (msgId: string, choiceIdx: number) => {
    if (!career) return;
    const c: NbaCareerState = JSON.parse(JSON.stringify(career));
    const line = answerNbaInboxMessage(c, msgId, choiceIdx);
    if (line === null) return;
    setCareer(c);
  ...
  /* Round 525: dismissing a pending rivalry beat applies its effect and
     clears the card, the same {state, log} shape buyNbaItem already
     returns. */
  const dismissRivalry = () => {
    if (!career) return;
    const { state, lines } = dismissNbaRivalryEvent(career);
    setCareer(state);
  ...
  /* ------------------- Round 525: a pending rivalry beat -------------------
     ...
     the same place in the flow the flagship's and the NFL board's own
      ...
      <div className="space-y-4">
        ...
        <div className="relative rounded-2xl border border-gold/50 bg-card p-5 text-center">
          ...
          <div className="mt-3 space-y-1 text-xs text-muted-foreground">
            ...
            {(() => {
              const earned = nbaEarnedBadges(career);
      ...
      <div ref={panelRef} className="space-y-3">
        ...
        {panel === 'bank' && (
          /* Round 470: the money app. Savings, the market, the statement and
             ...
             Round 57 shop as its fourth tab. */
          <MoneyApp
            host={career}
            sport={NBA_MONEY}
            ...
            onMoney={handleMoney}
            shop={<NbaShopPanel career={career} onBuy={id => { const res = buyNbaItem(career, id); if (!res) return; setCareer(res.state); setFeed(f => [res.log, ...f].slice(0, 8)); persist(res.state, 'season', teamQuality); }} />}
          />
        )}
        {panel === 'stats' && (
        ...
        {panel === 'trophies' && (
          <div className="space-y-3">
            ...
            <BadgeGrid defs={NBA_BADGES} earned={nbaEarnedBadges(career)} />
          </div>
        )}
        {panel === 'news' && (
          <div className="space-y-3">
            ...
            {newsTab === 'fans' && (
              <SocialGram
                followers={fmtFollowers(nbaFollowers(career))}
                standing={career.fanbase}
                standingLabel="Fanbase"
                comments={nbaFanComments(career)}
                headlines={(career.headlines ?? []).slice(0, 3)}
              />
            )}
            {newsTab === 'rival' && (
              <RivalCard rival={career.rival} myName={career.name} teamLabel={nbaTeamLabelOf(career.rival?.team ?? '', career.eraId)} ringWord="ring" />
            )}
          </div>
        )}
        {panel === 'inbox' && (
  ...
  const hubTiles = careerHubTiles({
    ...
    netWorth: Math.round(((career.netWorth ?? 0) + nbaMoneyWealth(career)) * 10) / 10,
    salary: career.salary,
    yearlyCosts: career.yearlyCosts ?? 0,
    contractYears: Math.max(0, career.contractYears),
    teamLabel: nbaTeamLabelOf(career.team),
    ...
    rings: career.rings,
    ringWord: 'ring',
    honours: [{ label: 'MVPs', n: career.mvps }, { label: 'All-NBA nods', n: career.allNbas }],
  ...
  /* Round 525: a sixth box for the inbox, appended locally rather than
     ...
     MLB and NHL hubs, neither of which has an inbox to show yet, and a box
     with nothing behind it is worse than no box. */
  const unread = nbaUnreadInboxCount(career);
