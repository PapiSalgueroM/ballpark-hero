...
/* Round 470: the money app, the gram, the rival card and the badges, on the
   ...
   careerSocial, careerBadges), bound to hockey in nhlCareerMoney.ts and
   nhlCareerLoop.ts. */
...
export default function NhlMyCareerBoard() {
  ...
  useEffect(() => {
    try {
      ...
      setCareer(repairNetWorth(s.c, id => getNhlSpendItem(id)?.cost ?? 0));
      ...
      if (restoredPhase === 'retired') markRestoredFinish('nhl-my-career');
  ...
  const create = () => {
    ...
    setDraftDay({ pick: c.draftPick, teamLabel: nhlTeamLabelOf(c.team), playerName: c.name, lines: [pressureLine, roleNote], firstRoundEnd: 30 });
    setPhase('season');
    persist(c, 'season', tq);
  };
  ...
  const playSeason = () => {
    ...
    if ((c.suspendedSeasons ?? 0) > 0) {
      ...
      const banned: NhlSeasonLine = {
        year: c.year, team: c.team, age: c.age, ovr: c.ovr, games: 0,
        awards: [], teamResult: 'SUSPENDED', salary: 0,
      };
      c.seasons.push(banned);
      c.headlines = pushHeadlines(c.headlines, nhlHeadlinesFor(c, banned));
      const banNotes = nhlProgress(c, Math.random);
      setLastLine(banned);
      setCareer(c);
      setFeed(['🚫 Season served on the suspended list. No hockey, no money, no going back.', ...banNotes]);
      /* Round 186: even a banned year gets its card, muted on purpose. */
      setReveal(buildSeasonReveal({
        year: banned.year,
        subHeader: `${nhlTeamLabelOf(banned.team, c.eraId)} · age ${banned.age} · ${c.pos}`,
    ...
    if (extensionDue(c) && !extDeclinedRef.current) {
      setExtTalk(buildNhlExtension(c, Math.random));
    ...
    c.headlines = pushHeadlines(c.headlines, nhlHeadlinesFor(c, line));
    setLastLine(line);
    /* Round 186: the curtain. Every string in it is one the engine already
       wrote; the true stat line is on screen from frame one. */
    setReveal(buildSeasonReveal({
      year: line.year,
      subHeader: `${nhlTeamLabelOf(line.team, c.eraId)} · age ${line.age} · ${c.pos}`,
  ...
  const pushExt = () => {
    ...
    setExtTalk(pushExtension(extTalk, nhlExtPushArgs(career, Math.random)));
  };
  ...
  const handleMoney = (action: MoneyAction) => {
    if (!career) return;
    const c: NhlCareerState = JSON.parse(JSON.stringify(career));
    const res = nhlMoneyAct(c, action);
    if (!res.ok) return;
    setCareer(c);
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="mx-auto max-w-md space-y-3">
          ...
          <AppearanceBuilder appearance={appearance} onChange={setAppearance} clubColor="#0EA5E9" />
      ...
      <div ref={panelRef} className="space-y-3">
        ...
        {panel === 'bank' && (
          /* Round 470: the money app. Savings, the market, the statement and
             ...
             Round 59 shop as its fourth tab. */
          <MoneyApp
            host={career}
            sport={NHL_MONEY}
            ...
            onMoney={handleMoney}
            shop={<NhlShopPanel career={career} onBuy={id => { const res = buyNhlItem(career, id); if (!res) return; setCareer(res.state); setFeed(f => [res.log, ...f].slice(0, 8)); persist(res.state, 'season', teamQuality); }} />}
          />
        )}
        {panel === 'stats' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-2 text-center text-[11px]">
              ...
              <div className="rounded-xl border border-border bg-card px-2 py-2"><p className="text-lg font-black text-foreground">{career.cups}</p><p className="text-muted-foreground">Cups</p></div>
            </div>
          </div>
        )}
        {panel === 'log' && (
        ...
        {panel === 'trophies' && (
          <div className="space-y-3">
            <TrophyCase seasons={career.seasons} rings={career.cups} ringWord="Cup" />
            {/* Round 470: the peaks between the trophies. Evaluated on every
                render off the save, so a badge can never be stale. */}
            <BadgeGrid defs={NHL_BADGES} earned={nhlEarnedBadges(career)} />
          </div>
        )}
        {panel === 'news' && (
          <div className="space-y-3">
            ...
            {newsTab === 'fans' && (
              <SocialGram
                followers={fmtFollowers(nhlFollowers(career))}
                standing={career.fanbase}
                standingLabel="Fanbase"
                comments={nhlFanComments(career)}
                headlines={(career.headlines ?? []).slice(0, 3)}
              />
            )}
            {newsTab === 'rival' && (
              <RivalCard rival={career.rival} myName={career.name} teamLabel={nhlTeamLabelOf(career.rival?.team ?? '', career.eraId)} ringWord="Cup" />
            )}
          </div>
        )}
        {panel === 'inbox' && (
  ...
  const hubTiles = careerHubTiles({
    ...
    netWorth: Math.round(((career.netWorth ?? 0) + nhlMoneyWealth(career)) * 10) / 10,
    salary: career.salary,
    yearlyCosts: career.yearlyCosts ?? 0,
    contractYears: Math.max(0, career.contractYears),
    teamLabel: nhlTeamLabelOf(career.team),
    ...
    rings: career.cups,
    ringWord: 'Cup',
    honours: [{ label: 'Major awards', n: career.harts }, { label: 'All-Star nods', n: career.allStars }, { label: 'Conn Smythes', n: career.connSmythes }],
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-center gap-2 text-xs">
        {career.appearance && (
          <span className="overflow-hidden rounded-xl border border-border bg-card">
            <PlayerAvatar appearance={career.appearance} clubColor="#0EA5E9" size={44} animate />
          </span>
        )}
      ...
      {((career.heat ?? 0) > 0 || (career.dirtyMoney ?? 0) > 0) && (() => {
        const hv = career.heat ?? 0;
        const band = nhlHeatLabel(hv);
        return (
          <div className="rounded-2xl border border-destructive/25 bg-destructive/5 p-3">
            <div className="flex items-center justify-between text-[11px]">
              <span className="font-bold uppercase tracking-wider text-muted-foreground">🕶️ League office</span>
              <span className={cn('font-black', band.tone)}>{band.label}</span>
            </div>
            <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-secondary">
              <div className={cn('h-full rounded-full transition-all duration-700', hv >= 65 ? 'bg-destructive' : hv >= 40 ? 'bg-orange-500' : 'bg-gold')} style={{ width: `${hv}%` }} />
            </div>
...
/* ─── Round 59: the money panel ───
   Seven aisles of things to spend it on, plus a Shady aisle that only shows up
   once you actually have something to hide. Gates mirror buyNhlItem exactly so
   a button never lies about what it will do. */
function NhlShopPanel({ career, onBuy }: { career: NhlCareerState; onBuy: (id: string) => void }) {
  const [tab, setTab] = useState<NhlSpendCategory>('home');
  const cats: { key: NhlSpendCategory; label: string; emoji: string }[] = [
  ...
  const items = NHL_SPEND_ITEMS.filter(i => i.category === tab);
