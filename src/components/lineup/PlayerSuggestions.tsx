...
const PlayerSuggestions = ({ query, teamName, isNation, onSelect, visible }: PlayerSuggestionsProps) => {
    ...
    debounceRef.current = window.setTimeout(async () => {
      ...
      try {
        const resp = await fetch(
          `${"https://flawuiqbvjobmkfkauhw.supabase.co"}/functions/v1/suggest-players`,
          {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ query, teamName, isNation }),
          }
        );
  ...
  }, [query, teamName, isNation, visible]);
