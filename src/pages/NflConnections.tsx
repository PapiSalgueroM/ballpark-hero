...
const NflConnections = () => {
  ...
  return (
    <>
      <PageSeo
        title="NFL Connections - Football Player Grouping Puzzle | DoUKnowBall"
        description="Find four groups of 5 NFL players that share a connection. Same franchise, college, or milestone. Daily challenge."
      ...
      >
        ...
        {!isLoading && gameStatus === 'complete' && (
          <div className="mt-6 flex justify-center">
            <ResultScreen
              ...
              emojiGrid={lives > 0 ? `🏆 NFL Connections: all 4 groups, ${lives} ${lives === 1 ? 'life' : 'lives'} left` : `🏈 NFL Connections: ${solvedGroups.length}/4 groups`}
              share={{
                score: lives > 0 ? `all 4 groups with ${lives} ${lives === 1 ? 'life' : 'lives'} left on today's NFL Connections` : `today's NFL Connections`,
                gameName: 'NFL Connections',
                gamePath: '/nfl-connections',
              }}
        ...
        <GameSeoContent
          ...
          howToPlay={[
            'Find four groups of 5 NFL players that share a connection',
          ...
          examples={[
            'Drafted #1 overall: Matthew Stafford, Andrew Luck, Joe Burrow...',
            'Played college ball at LSU: Ja\'Marr Chase, Justin Jefferson, OBJ...',
            '55,000+ passing yards: Drew Brees, Philip Rivers, Eli Manning...',
            'Played for the Patriots: Gronk, Edelman, Jimmy G...',
          ]}
        />

        <AdBanner slot="1234567907" format="horizontal" className="mt-8" />

        <div className="flex justify-center mt-6">
          <ReportQuestion gameType="nfl-connections" gameContext={{ puzzleId: puzzle?.id }} />
        </div>
        <GameNav />

        <NflConnectionsHowToPlay open={showRules} onOpenChange={setShowRules} />
      </GameShell>
    </>
  );
};

export default NflConnections;
