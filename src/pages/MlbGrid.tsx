...
const MlbGrid = () => {
  ...
  const [gridData, setGridData] = useState<MlbGridData | null>(null);
  const [mode, setMode] = useState<Mode>('daily');

  const boot = useCallback(async () => {
    setPhase('boot');
    const d = await fetchMlbGridData();
    if (!d) {
      setPhase('error');
      return;
    }
  ...
  const dailyStorageKey = `mlb-grid-daily-${todayStr}`;
  ...
  return (
    <>
      ...
      <GameShell help="none"
        ...
        subtitle="Legends era: every career here finished by 2019. 9 cells, 9 guesses."
        headerExtra={
          <>
            <HowToPlayPopover title="How to Play MLB Franchise Grid">
              <p className="text-muted-foreground text-center">
                Fill all 9 cells of the grid with real MLB players.
              </p>
              <section>
      ...
      >
        ...
        {phase === 'error' && (
          <div className="text-center py-12">
            <p className="text-destructive font-semibold mb-3">Couldn't load MLB career data right now.</p>
            ...
            {activeCell !== null && !gameOver && (
              <div className="mt-6 max-w-md mx-auto">
                ...
                <PlayerAutocomplete
                  ...
                  searchOptions={{ source: MLB_GRID_PLAYER_SOURCE, exclude: guessedNames }}
                  placeholder="Type an MLB legend..."
                  validateOnly
                  autoFocus
                />
              </div>
        ...
        <div className="flex justify-center mt-6">
          <ReportQuestion gameType="mlb-grid" />
        </div>

        {/* Round 358: the archive is reachable from the game it belongs to,
            so it is part of the site rather than a page only a sitemap knows. */}
