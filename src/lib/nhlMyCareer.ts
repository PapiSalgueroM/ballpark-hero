...
import { nhlMoneySeasonTick } from './nhlCareerMoney';
/* Round 525: the inbox and the rivalry events, the same lift Round 521 gave
   ...
   NhlCareerState type only, so there is no cycle at runtime: both ticks are
...
export interface NhlCareerState {
  ...
  /** Round 470: the savings account, the market and the statement, on the
      ...
      lazily by ensureNhlMoney, so an old save opens on every screen with its
      market at par. */
  money?: MoneyState;
  ...
  /** Round 525: the inbox, on the same engine the flagship's phone runs
      (careerInbox.ts), bound to the NHL in nhlCareerInbox.ts. Absent on a
  ...
  /** Round 525: the rivalry events, on the same engine the flagship's
      ...
      nhlCareerRivalryEvents.ts. The SAME rival draftRival already drafts;
...
export function simNhlSeason(
  ...
  /* Round 525: the rivalry beat, rolled right after the rival's own season,
     ...
     dismissNhlRivalryEvent, never here, so the state this function hands
