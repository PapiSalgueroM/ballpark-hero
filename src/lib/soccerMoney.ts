/* ─── Round 134: money you can get wrong ────────────────────────────────────

   What money was before this file. Net worth went up every season by wages
   ...
   So this file is a market. Five things you can put money into, each with a
   price that moves on its own every season whether you are watching or not,
   plus a savings account that pays a small safe rate. You buy units at today's
   price and you sell them at some later price, and the gap is the whole game.

   THE FIVE RULES THIS FILE LIVES BY

   1. Its own random stream. PhoneState has its own seed so that replying to a
      text does not shift the world sim's Math.random stream and break the
      paired A/B in simPhone2. Buying and selling has exactly the same problem,
      so MoneyState carries its own seed too. Nothing in this file ever calls
      Math.random.

   2. Prices mean revert toward a moving anchor. A pure random walk cannot be
      timed: every price is as good as every other price and "buy low sell
      high" is superstition. Pulling the price back toward an anchor is what
      makes buying a dip a real decision and holding a spike a real mistake.
      The anchor itself drifts upward for the boring assets and sits flat for
      the risky one, which is why the fund quietly compounds and the coin does
      not.
   ...
   3. Nothing here is a printer. Fees on both sides of a trade, a savings rate
      ...
      lifestyle bill that counts money hidden in the vault. Guarded by
      scripts/simMoney.mjs over hundreds of careers.
   ...
   4. Bounded storage. Prices keep MAX_HISTORY points, the ledger keeps
      ...
      assets. The whole of MoneyState is under a kilobyte for a 20 season
      career and it does not grow with career length.

   5. Old saves. ensureMoney is pure and repairs anything, so a save from
      before this round opens on any screen, at any phase, and starts its
      market at par on the season it is on.
*/
...
/** Transactions kept. He asked for a limit on how many show, and a phone
 *  screen holds about this many without turning into a page you scroll.
 *  Everything older is gone rather than hidden, because a ledger nobody can
 *  see is just save size. */
...
export interface MoneyState {
  ...
  won: number;
  lost: number;
  /** The card school on the team bus. See cardsPlay. */
  cPlays: number;
  cNet: number;
  cYear: number;
  cShut: boolean;
...
export function ensureMoney(s: CareerState): MoneyState {
  const raw = (s as CareerState & { money?: Partial<MoneyState> }).money;
  /* The seed for a save that has never had one is derived from the career
     itself rather than from Math.random, so opening the bank twice on the
     same save shows the same market both times. */
  const born = (s.playerName || "x").split("").reduce((h, c) => (h * 31 + c.charCodeAt(0)) >>> 0, 7);
  const out = blankMoney(born ^ ((s.seasons?.length ?? 0) * 2654435761));
...
function writeMoney(s: CareerState, m: MoneyState): void {
  (s as CareerState & { money?: MoneyState }).money = m;
}
...
export function moneySeasonTick(s: CareerState, year: number): MoneyTick {
  ...
  for (const def of ASSETS) {
    ...
    if ((m.hold[def.id] ?? 0) > 0 && events.length < 2) {
      ...
      if (move >= 0.28) {
        events.push(`${def.emoji} ${def.name} jumped ${Math.round(move * 100)}%. Your stake is worth ${fmtM(holdingValue(m, def.id))} now.`);
      } else if (move <= -0.25) {
        events.push(`${def.emoji} ${def.name} dropped ${Math.round(-move * 100)}%. Your stake is down to ${fmtM(holdingValue(m, def.id))}.`);
      }
    }
  }
  ...
  if (s.netWorth < CASH_FLOOR) {
    let need = r2(CASH_FLOOR - s.netWorth);
    if (m.vault > 0 && need > 0) {
      const take = Math.min(m.vault, need);
      m.vault = r2(m.vault - take);
      s.netWorth = r2(s.netWorth + take);
      need = r2(need - take);
      note(m, year, "Savings covered the bills", -take);
      events.push(`🏦 Bills came out of savings this season. ${fmtM(take)} gone.`);
    }
    ...
    for (const def of order) {
      ...
      s.netWorth = r2(s.netWorth + got);
      need = r2(need - got);
      events.push(`📉 Had to sell ${def.name} to cover the bills. ${fmtM(got)} raised at a bad price.`);
    }
  }
  ...
  /* 4. Money is not a scoreboard. Being skint is stressful and having a real
        ...
        rather than added up season after season, the way the phone's standing
        does it, so twenty seasons of either never rails him. */
  const buffer = m.vault + investedValue(m);
  const bill = Math.max(0.2, s.lifestyleCostPerYear || 0.2);
  if (s.netWorth + buffer < bill * 0.75) {
  ...
  /* 5. The bus. Somebody who keeps sitting in and keeps losing hears about it
        at home, once, and then the invitations stop. */
  if (m.cPlays >= 4 && m.cNet <= -0.12 && !m.cShut && m.cYear === year) {
    s.morale = clamp(s.morale - 3, 0, 100);
    s.karma = clamp((s.karma ?? 50) - 2, 0, 100);
    events.push(`🃏 Somebody at home asked how much you have actually lost on that bus. You did not have a good answer.`);
  }
  if (m.cNet <= -0.5 && !m.cShut) {
    m.cShut = true;
    events.push(`🃏 The lads have stopped dealing you in. Honestly, that is probably for the best.`);
  }
...
export function spendable(s: CareerState): number {
  return r2(Math.max(0, s.netWorth - CASH_FLOOR));
}

export function fmtM(v: number): string {
  const neg = v < 0;
  const a = Math.abs(v);
  const body = a >= 1000 ? `€${(a / 1000).toFixed(2)}bn` : a >= 1 ? `€${a.toFixed(1)}M` : `€${Math.round(a * 1000)}k`;
...
export function moneyAct(s: CareerState, action: MoneyAction): MoneyOutcome {
  ...
  if (action.t === "deposit") {
    ...
    s.netWorth = r2(s.netWorth - amount);
    m.vault = r2(m.vault + amount);
    note(m, year, "Into savings", -amount);
    writeMoney(s, m);
    return { ok: true, event: null, toast: `${fmtM(amount)} into savings` };
  }
  ...
  if (action.t === "withdraw") {
    ...
    writeMoney(s, m);
    return { ok: true, event: null, toast: `${fmtM(amount)} out of savings` };
  }
  ...
  if (action.t === "buy") {
    ...
    return {
      ok: true,
      event: `${def.emoji} Put ${fmtM(amount)} into ${def.name} at ${Math.round(price)}.`,
      toast: `Bought ${fmtM(amount)} of ${def.name}`,
    };
  }
  ...
  if (action.t === "sell") {
    ...
    if (cash <= 0) return NO;
    s.netWorth = r2(s.netWorth + cash);
    writeMoney(s, m);
    const profit = r2(cash - basis * frac);
    const verb = profit >= 0 ? "up" : "down";
    return {
      ok: true,
      event: `${def.emoji} Sold ${fmtM(before * frac)} of ${def.name}, ${verb} ${fmtM(Math.abs(profit))} on what you paid.`,
      toast: `Sold for ${fmtM(cash)}, ${verb} ${fmtM(Math.abs(profit))}`,
    };
  }

  if (action.t === "cards") return cardsPlay(s, m, action.stake, year);
  if (action.t === "arcade") return arcadeScore(s, m, action.right, year);
  return NO;
}

/* ─── the card school on the team bus ────────────────────────────────────────
   ...
   of every team bus in the world.
   ...
   down and it gets noticed at home, once, and that costs morale and karma.

   And it ends. Once you are down CARD_SHUT the lads stop dealing you in, for
...
export function cardCap(s: CareerState): number {
  const m = ensureMoney(s);
  if (m.cShut) return 0;
  if (s.netWorth < CARD_MIN_WORTH) return 0;
  return r2(Math.max(0, Math.min(CARD_MAX, s.netWorth * CARD_FRACTION)));
}
...
function cardsPlay(s: CareerState, m: MoneyState, stake: number, year: number): MoneyOutcome {
  ...
  if (win) {
    ...
    s.netWorth = r2(s.netWorth + profit);
    m.cNet = r2(m.cNet + profit);
    note(m, year, "Cards on the bus", profit);
    event = `🃏 Took ${fmtM(profit)} off the lads on the bus. They will want it back.`;
    toast = `You won ${fmtM(profit)}`;
  } else {
    s.netWorth = r2(s.netWorth - bet);
    m.cNet = r2(m.cNet - bet);
    note(m, year, "Cards on the bus", -bet);
    event = `🃏 Lost ${fmtM(bet)} at cards on the bus. That is how it usually goes.`;
    toast = `Lost ${fmtM(bet)}. That is the usual outcome`;
  }
...
function arcadeScore(s: CareerState, m: MoneyState, right: number, year: number): MoneyOutcome {
  ...
  let event: string;
  if (got === 3) {
    s.netWorth = r2(s.netWorth + ARCADE_PRIZE);
    s.morale = clamp(s.morale + 2, 0, 100);
    note(m, year, "Won the phone quiz", ARCADE_PRIZE);
    event = `🎮 Three from three on the ball quiz. ${fmtM(ARCADE_PRIZE)} and bragging rights in the group chat.`;
  } else if (got === 2) {
    s.morale = clamp(s.morale + 1, 0, 100);
    event = `🎮 Two from three on the ball quiz. The lads are not impressed.`;
  } else {
    event = `🎮 ${got} from three on the ball quiz. You play the sport for a living.`;
  }
  writeMoney(s, m);
...
export function bankSummary(s: CareerState): BankSummary {
  ...
  return {
    ...
    vault: m.vault,
    invested,
    total: r2(s.netWorth + m.vault + invested),
    won: m.won,
    lost: m.lost,
