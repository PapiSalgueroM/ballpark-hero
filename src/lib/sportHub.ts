...
export const SPORT_HUBS: SportHub[] = [
  {
    ...
    referenceLinks: [
      { path: '/records#epl', label: 'English champions, year by year', why: 'every English league title winner in the table, year by year, checked against the record' },
  ...
  },
  {
    ...
    whyHere:
      'Eleven games, and they mostly split by how much of your evening they want. The quick ones are guessing games with different tempers: Puck Detective hides a player off a current roster and answers each of your eight guesses with five chips, NHL Career Path gives you a position and then charges points for every clue you buy, NHL Higher or Lower asks only who finished with more career points. NHL Franchise Grid and NHL Connections test breadth instead, nine cells against twenty names. The long ones simulate: NHL Front Office is a real 2026-27 roster under a hard cap, NHL My Career follows one fictional player for up to twenty two seasons, NHL Conquest turns a season of results into a land grab. 82-0 NHL Perfect Season and Perfect Lineup: NHL draft then simulate, and NHL Connect 4 needs a second person.',
    startHere: [
      { path: '/puck-detective', label: 'Puck Detective', why: 'Pick this if you watch the league right now. The answer is always somebody on a current roster, and every guess comes back with five chips (team, position, nationality, age, jersey number), so you narrow instead of fishing. NHL Career Path is the same idea for people whose hockey is older than this season.' },
      { path: '/hockey-career', label: 'NHL Career Path', why: 'The all-time version of the same puzzle. You open with nothing but a position and buy clues one at a time (country, draft, teams, stats, awards), each one shaving your score, so it rewards recognising a draft slot rather than knowing depth charts. Wrong guesses are free here, which Puck Detective\'s eight guess limit is not.' },
      { path: '/perfect-lineup-nhl', label: 'Perfect Lineup: NHL', why: 'Five minutes, and no season to sit through. Three of your six slots arrive locked to a franchise or a decade, you fill them from the all-time pool, and one simulated game hands back a letter grade. 82-0 NHL Perfect Season is the longer cousin: a wheel decides your options for you and the sim plays all 82 games.' },
  ...
  },
  {
    ...
    whyHere:
      'Three of these run long. NFL Front Office is the desk job: every club in the league, a hard cap, trade partners who hang up on you, an owner who fires you when the mandate slips. NFL My Career runs the same league from the locker room, eight positions, and an envelope that gets passed down the row in a meeting: put your money in and the heat starts climbing toward league security and a file with your name on it. 17-0 NFL Perfect Season drafts one cross era offense off a wheel of real team seasons and simulates whether it runs the table. The other seven are short: NFL Grid, NFL Connections, NFL Connect 4, NFL Career Path and NFL Higher or Lower are recall in five different shapes, Missing Eleven needs ninety seconds to make you doubt a game you watched live, and NFL Conquest plays a season as a land grab.',
    startHere: [
      { path: '/front-office', label: 'NFL Front Office', why: 'The GM chair, and the open ended one: it runs for as many seasons as ownership lets you keep the job. Real rosters against a hard cap, trades where the other side counters or hangs up, draft grades that lie to you. NFL My Career is the same league from a player\'s seat, and 17-0 NFL Perfect Season is one team drafted for one run.' },
    ...
    referenceLinks: [
      { path: '/records#sb', label: 'Super Bowl champions, year by year', why: 'every Super Bowl with the runner-up, the score, the MVP and the venue, checked against the record' },
    ...
    hubFaqs: [
      ...
      { q: 'I have only really watched since about 2010. Am I going to be lost?', a: 'Not on most of them. NFL Higher or Lower only uses careers that started in 2000 or later, NFL Front Office is the modern league rather than a history test, and the 17-0 NFL Perfect Season wheel starts at 1999. Some of the others reach a long way further back, and those are the ones where you actually pick something up.' },
  ...
  },
  {
    ...
    whyHere:
      'Three shapes and an outlier. Straight recall: MLB Higher or Lower is one question on a loop (which legend hit more career home runs), MLB Career Path walks a mystery player\'s card down a clue ladder until you can name him, and Missing Nine hands you a famous World Series order with a hole in it and lets the gap do the work. Puzzles that make you hold two facts at once: MLB Franchise Grid wants a name that answers both of a square\'s headings at the same time, MLB Connections wants the thread running through a set of names, MLB Connect 4 buries trivia under the discs. Then the sims, where one sitting is a season or a life: MLB Front Office runs a club down to the tax line, MLB My Career runs one player you create, and 162-0 MLB Perfect Season drafts across eras. The outlier is MLB Conquest, a daily of its own shape: you pick a club, call its game each round, and the simulated results across all 30 redraw a 58-territory map where the winner annexes the loser\'s whole empire. Toronto and San Diego start with no land at all.',
    startHere: [
      { path: '/baseball-career', label: 'MLB Career Path', why: 'Start here if you want the guessing kind. It reads you the back of a baseball card a line at a time, the clubs he passed through, the numbers he put up, the trophies on the shelf, until one of them tips it and the name lands. A wrong guess does not move the ladder and does not dock you, so an early stab costs you nothing and the rest of the card is still ahead of you. Missing Nine, the other guessing game, expects you to already know a specific famous team.' },
      ...
      { path: '/mlb-grid', label: 'MLB Franchise Grid', why: 'Take this over MLB Connections if you like the crossword shape. Every square sits where a row heading meets a column heading and only a name that answers both will do, which quietly makes the odd corner of a career worth more than the biggest name you know. MLB Connections is the opposite job: you are handed the names and have to work out what already links them.' },
      { path: '/mlb-front-office', label: 'MLB Front Office', why: 'The long one to pick if you would rather run the club than be the player: real 2026 rosters, payroll against the tax line, deadline trades, October, then doing it again next year. MLB My Career is its mirror image, one player you create from draft day, and 162-0 MLB Perfect Season is neither, a draft across eras chasing an unbeaten season no real team has ever had.' },
    ],
    reference:
  ...
  },
  {
    ...
    referenceLinks: [
      { path: '/records#cfb', label: 'College football national champions', why: 'every season with the selector, the result and the coach, split titles kept split' },
      { path: '/records#cbb', label: "Men's NCAA basketball champions", why: 'every title game since 1939 with the beaten finalist and the score' },
