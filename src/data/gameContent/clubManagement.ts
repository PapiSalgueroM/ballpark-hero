...
export const CLUB_MANAGEMENT_CONTENT: GameContentMap = {
  '/club-manager': {
    ...
    howToPlay: [
      ...
      "Work the free agent board on the Free tab whenever you like, window or no window: players with no club cost no fee, only a signing on fee and a wage, and they are the only signings you can make in October or February. Or go the other way and pay a contract off on the contracts desk, which buys out what you still owe him and turns him into somebody else's free agent.",
    ...
    rules: [
      ...
      "Free agents are a market of their own and the transfer window does not touch them, because there is no selling club and so no registration being traded between clubs. They cost a signing on fee and a wage instead of a fee, and the wage is the catch: with nothing going to a club the player and his agent take that money, so a free agent runs about a third dearer a week than the same man with a price on him, and the board refuse a wage that takes you far past their ceiling. The board is mostly players past thirty and squad filler, which is who really comes free, and rival clubs work the same list every week. A player you terminate or let walk joins it, and cannot re-sign for you until the summer.",
      "Terminating a contract buys back the wages you still owe: a quarter of them for a man agitating to leave, up to most of them for a contented first teamer, never free and never the full amount. It comes out of the transfer kitty and shows on the finances desk as contract pay offs, his wage leaves the bill that week, and the dressing room reacts to who you cut. You cannot pay off a loanee, your last goalkeeper, or anybody if it would take the squad under 14.",
