...
export const SOCCER_CONTENT_2: GameContentMap = {
  ...
  '/soccer-conquest': {
    intro: [
      "Soccer Conquest has two actual modes. Daily Season puts 96 clubs from the Premier League, La Liga, Serie A, the Bundesliga and Ligue 1 on one map of Europe. England Attack puts 20 English clubs on a generated geographic map for an unlimited last-club-standing run.",
      "Daily Season strength comes from the squad market values on record. England Attack uses simulated player ratings for its matches and upgrades, saves every committed move in a separate browser slot, and awards no ranked points.",
    ],
    howToPlay: [
      "Choose Daily Season or Attack. An unfinished Daily Season opens first, and either mode stays available from the mode buttons.",
      "Daily Season: pick your club from all 96, grouped by league. Each matchday pairs the whole continent into 48 games, and you call your club's result before the map redraws.",
      "Daily Season: survive 10 matchdays. The top 8 empires by region make the playoffs, with record breaking ties, then the knockout crowns the winner.",
      "Attack: start or resume a saved England run. Spin for a club that still owns land, then spin one of the legal compass directions from its home or nearest playable frontier.",
      "Attack: claim neutral ground or play the first club reached by the ray. A match winner takes every region the loser held, plus that club's best original player and any players it captured earlier.",
      "Keep spinning until one club owns the whole generated map. Finished Attack saves stay available on reload, and a new run starts only when you ask for one.",
    ],
    rules: [
      "Daily Season has 10 regular matchdays, then an 8 club knockout bracket.",
      "Daily Season has 154 regions: 96 club home areas plus the countryside between them, which opens in the hands of the nearest club of the same country.",
      "Daily Season scoring is 25 points per correct call, 3 per region held at the end, 200 for the crown and 50 for making the playoffs.",
      "In Daily Season, wiped-out clubs keep playing, one win can take an empire back and level games go to penalties.",
      "Daily Season strength is the 2026 squad market value on record, mapped onto the same 55 to 95 band the other conquest maps use. Clubs with limited coverage are marked partial data.",
      "The Daily Challenge deals every player the same date-seeded season: same opening map, same fixtures and same results. One scored run per day builds streaks. Free Play is unlimited and fully random.",
      "England Attack starts 20 clubs on generated game regions clipped to England. The borders are game regions, not real administrative lines, and only directions that cross connected land to another region can be spun.",
      "Attack uses simulated ratings. Neutral expansion improves one top-rated player by 2, completing the map improves one by 4, and all ratings stop at 99.",
      "Attack is unlimited and gives no ranked points. Its separate save survives reload without rerolling a committed club, direction, result or finished map.",
    ],
    example: [
      "Daily Season example: you call your club's result, watch all 48 games redraw the European map, then try to hold enough of the 154 regions to reach the top 8 after matchday 10.",
      "Attack worked example: fictional Amber Vale spins east into a neutral region. It claims that land, its best player gains 2 simulated rating points, and the next turn starts with another club spin.",
      "On the next Attack, Amber Vale hits fictional Birch Town. If Amber Vale wins the simulated match, Birch Town is eliminated, its regions change owner and its captured-player chain moves to the winner.",
    ],
    tips: [
      "In Daily Season, use the displayed odds and check the standings late. Playoff seeding goes by regions, so the last matchdays are about protecting your count.",
      "A wiped-out Daily Season club can still take an empire back. That comeback rule does not apply to eliminated clubs in Attack.",
      "In Attack, inspect the map and club list before each spin. A club whose home route is blocked launches from its nearest owned frontier with a legal direction.",
      "Attack saves after each committed action. If safe storage is blocked or damaged, the page says so and will not silently replace the old bytes.",
    ],
    faqs: [
      ...
      { q: "Where do the strengths come from?", a: "From the 2026 squad market values on record for each club, summed and mapped onto a 55 to 95 band. Nothing is hand rated. Clubs with limited coverage are marked partial data." },
      ...
      { q: "How does England Attack work?", a: "Spin a living club, spin a legal direction, then claim neutral land or play the club in the ray's path. Match winners eliminate the loser, take its regions and carry over its captured players. The last club standing rules England." },
      { q: "Does Attack affect my daily score or streak?", a: "No. Attack is unlimited, unranked and saved separately. Daily Season keeps its one scored run, points and streak rules." },
      { q: "Are Attack ratings real?", a: "No. Attack powers, match ratings and upgrades are simulated game values. The current English slice reuses the existing documented club and roster assets rather than inventing missing players." },
      { q: "Will Attack reroll when I reload?", a: "No. Every committed phase is saved, including the spun club, direction, target, result and finished map. A blocked or damaged save is shown instead of being silently replaced." },
    ],
  },
