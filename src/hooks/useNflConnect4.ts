...
import { curatedBoards, getRandomConnect4Board } from '@/data/nflConnect4Boards';
...
import type {
  ...
  Connect4Board,
  Connect4WinInfo,
} from '@/types/nflConnect4';

/**
 * NFL Connect 4, port of useMlbConnect4 (task #22 follow-on) against the
 * nfl-connect4-validate edge function (attribute-pair AI validator).
...
export function useNflConnect4() {
    ...
    async (playerName: string) => {
      ...
      try {
        const resp = await fetch(
          `${SUPABASE_URL}/functions/v1/nfl-connect4-validate`,
          {
            method: 'POST',
            headers: {
