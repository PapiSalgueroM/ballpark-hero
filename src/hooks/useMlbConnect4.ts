...
import { curatedBoards, getRandomConnect4Board } from '@/data/mlbConnect4Boards';
...
import type {
  ...
  Connect4Board,
  Connect4WinInfo,
} from '@/types/mlbConnect4';

/**
 * MLB Connect 4, port of useNbaConnect4 (task #22) against the
 * mlb-connect4-validate edge function (attribute-pair AI validator).
 * Keep the connect-4 hooks in lockstep if the mechanic changes.
 */
const ROWS = 6;
const COLS = 7;
...
export function useMlbConnect4() {
    ...
    async (playerName: string) => {
      ...
      try {
        const resp = await fetch(
          `${SUPABASE_URL}/functions/v1/mlb-connect4-validate`,
          {
            method: 'POST',
            headers: {
  ...
  useGameCompletion('mlb-connect-4', phase === 'won' || phase === 'draw', phase === 'won' ? 500 : 200);
