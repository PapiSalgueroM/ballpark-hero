...
| Shared data layer and provenance | 58% | Round 420: Player Bingo now has 73 exact date-of-birth identities backed by a committed two-source receipt, excludes deceased Diogo Jota, and uses conservative age bounds where an exact birth date is not audited. The wider age and value backlog still waits on documented datasets. |
...
| Indexing and SEO | 45% | Round 420 keeps the complete crawler copy while preventing it from flashing before React, ships a fresh count-free social-card URL, and rebuilds all route snapshots and the derived sitemap ledger. |
| Profiles and leaderboard | 45% | Round 420 separates guest and account streak state, makes hydration failure safe, serializes account writes, and prevents a delayed completion from one account mutating another. Historical point repair remains an owner decision, now 596,072 points across 152 accounts. |
...
| | |
|---|---|
| **CURRENT HEAD, CHECKED 2026-09-02. THIS SUPERSEDES THE OLD HEAD ROWS BELOW.** | Round 420 is complete in this working head, rebased with the landed Round 416 and Round 418 Front Office work. It fixes the raw crawler-copy flash without hiding the no-JavaScript fallback, refreshes the count-free social card, isolates guest and signed-in streak state, audits Player Bingo age boundaries, repairs Stock Market retry and Transfer Path transitions, removes unsafe first-render randomness, retires six obsolete Boot Room endpoints, and fixes Stadium Tycoon's prerender loop and subsecond match clock. Exact TypeScript, the production SEO build, all 15 generated-site fences, focused unit tests, browser first-paint and snapshot checks, and every new control passed. The full `runAllSims` pass is the remaining branch-level release gate and is not claimed complete in this row. Round 419 remains actively claimed by the desktop lane and had not landed at this check; Round 411 also remains claimed separately. Search Console accepted the sitemap on 2026-09-02 with status Success and 137 discovered pages, and all ten priority URLs entered its crawl queue. AdSense shows Getting ready and Review requested. Google's approval decision is pending. The long Round 362 and Round 257 rows below are preserved as history, not current AdSense guidance. |
...
- **2026-09-02, Round 420. THE FRONT DOOR AND ACCOUNT STATE TELL THE TRUTH.**
  ...
  Account hydration now has explicit pending, ready and failed states. Guest,
  account A and account B keep separate streak state; failed hydration permits
  local play without overwriting the profile backup; writes are serialized and
  identity captured, so a delayed completion from account A cannot mutate
  account B after a switch. Sign-out restores the guest. The account invitation
  also says plainly that an account saves streaks and enables leaderboard play.
  ...
  passes. The full `runAllSims` result is deliberately not claimed here until its
  final release run completes.
