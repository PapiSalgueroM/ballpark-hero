...
- CM-7 **MOSTLY DONE, Round 161: structured deals and deep filters.** Negotiations take
  ...
  STILL OPEN from his list: the nationality filter. The DB table HAS a nationality column
  (100 percent coverage on 2025 and 2026 rows, 147 to 152 distinct values, verified
  2026-08-19 via MCP), so it is a bake-script data pass plus a filter chip, the natural
  Round 194.
...
| Next free round number | **194** (check the folder first, the 3-hourly build task may have taken it) |
