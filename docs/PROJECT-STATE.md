...
| Next free round number | **244** (check the folder first before taking it) |
...
5. **Soccer Career: the club captaincy arc** (flagship, next up). Club captaincy today is a
   single random event (engine event id 3 sets isLeader) plus an under-23-gated dilemma;
   international captaincy has a real check (30+ caps, 80+ overall). The arc worth building:
   an EARNED club armband (age + tenure + standing), stripped on transfer with an honest
   handover line, a retention story in decline years, a cabinet line for seasons worn, and the
   existing isLeader growth effects folded in so there is one captaincy truth. Engine work in
   src/lib/soccerCareerEngine.ts (6,800 lines, read in slices), verified the R227 way: a
   seeded few-hundred-career harness measuring award age/tenure distributions, strip-on-
   transfer, and no contradiction with the random event path.
