...
layout controls pass. All fifteen final generated-site fences pass. The Conquest
four-file harness passes all 125 outcomes and 20 seeds with one worker, without
changing time limits. Its revision, short-run and extra-error controls verify
their intended failures. Round 514's snapshots and ledger are unchanged.
Production fixes are backed up at `42953e54` in draft PR67. Preview 4186 serves
the final bundle `index-ChbuCbOA.js`. The Conquest test-driver changes and final
verification notes are the following commit in this round. Round 515's full
node run continues separately, not here, so no whole-suite all-clear is claimed.
After that run ends, inspect its result before scheduling one combined-tree
full run. Do not duplicate an active run or rebuild the tree beneath it.
...
**Round 505 is merged on main at `d1c541b3`, publication not yet verified (Claude Code lane):
...
The owner's original pasted tweak list contains 198 atomic requests: 88 complete, 51 partial
and 59 open. That is 44.4% strictly complete. Counting each partial item as half complete gives
the most useful single estimate: 57.3% done and 42.7% left. Strictly, 55.6% still has at least
some work because partial items are not finished.

The much larger master plan contains 358 sections: 32 complete, 214 partial, 69 open and 43
policy-constrained sections. Removing the 43 constrained sections leaves 315 actionable ones.
On the same half-credit method, the actionable master plan is 44.1% done and 55.9% left. This
larger denominator is the honest answer to the owner's concern that the visible short list was
not everything still to do.
...
| P1 4, Champions League group count 4 vs the real 8 | DONE | Round 342 gave every era its real 32 club field and eight groups; Round 462 added the round of 16 the real competition had from 2003-04 to 2023-24 (group winners at home against runners-up, never the same group, never the same association, seeded so a reload replays the draw), keeps the final group tables on the Cups tab through the knockouts, and splits level points the way each league says (Spain and Italy head to head on a per pair results ledger, England and Germany goal difference then goals scored, France goal difference then head to head), with a save migration that opens a pre 462 save with an empty ledger and its round of 16 week repaired in place; fenced by simClubManagerEraUcl (twelve era careers, three controls) |
...
| Missing XI and Rarity Round: "many more puzzles", "keep the puzzles coming" | DONE | Round 463: 38 more real lineups (169 to 207), every eleven read from two independent sources on 2026-09-05 and every blanked man's spelling, nationality and club at the time checked by SQL; 20 more Rarity categories (16 to 36), each measured against the live tables, with Juventus, Bayern and three position tags rejected because the table would refuse right answers |
...
| | |
|---|---|
| **CURRENT HEAD, CHECKED 2026-09-05 NIGHT. THIS SUPERSEDES THE OLD HEAD ROWS BELOW.** | **Rounds 465 to 469** merged, gated, reviewed and live, on top of the 459 to 464 head below. The five: **465** Club Manager's two meters on every tab (the board meter is the sacking number itself, the fans track points per game at r = 0.89) and goals for and against on every league table; **466** a calendar you tap to sim to any day, on the one loop the four fast forwards now use, with windows marked and opponents named; **467** four facilities from level 1 to 10 with start levels derived from the club's stature, ticket and concession pricing with fan and board reactions, generated sponsors you can push, and a projected finances screen whose lines sum; **468** three Soccer Career training drills you actually play (wall shot, tackle, glove save) on the shared arcade engine, feeding growth within potential headroom; **469** the NFL career's money app, social feed, rival and badges lifted out of Soccer Career into shared modules with the NFL injected, ready for the NBA, MLB and NHL. The merge agent ran the full suite (227 pass, 1 red it then fixed) and five browser plays. The adversarial review that followed found three real defects past every gate, each fixed before the push: the finance desk could leave an employed manager's header reading Sacked, six of the eight bad sponsor brands were real companies, and two different fan moods with two vocabularies sat an inch apart on the hub. Eight smaller review findings landed with them, including a drills daily that could be dealt twice. What the review left open is under Open bugs. |
| HEAD CHECKED 2026-09-05 EVENING, SUPERSEDED | `f55e374f` plus the snapshot and docs commits on top of it = **Rounds 459 to 464** merged, gated, reviewed and live. The batch landed as the merge `70187d0f` of `owner-list-459-463` (five rounds built in parallel worktrees: **459** the Soccer Conquest, **460** Transfer Path rule modes, **461** Rebuild multiplayer, **462** Club Manager's era Champions League round of 16 and per league tiebreaks, **463** 38 Missing XI lineups, 20 Rarity categories, Who Am I 500 to 600 with a measured meter), then an adversarial review (ten lenses, three skeptics per finding, 32 agents finished before the session limit cut the rest) whose seven confirmed findings landed as `74eea19c` before anything was pushed, then `f55e374f` repaired three harnesses that had been red on main since Rounds 450 to 458 (a pin Round 458 removed on purpose, a title band list stale after the roster re-bake, an add-on rate read off a capped headline feed). The full suite ran on the integration worktree: 221 node harnesses, 6 red, of which 3 were those pre existing repairs, 1 was the review's slug finding (simDailyLegend, fixed), 1 the mlb_grid_players flake (green alone) and 1 (simInventedNames) a real name in Rebuild's manager bank, fixed. Type gate zero; every new harness green with every control firing; the 23 site fences green on main's fresh build; nine routes played clean in a real browser. What the review found and left open is under Open bugs. Next: Rounds 465 to 469, claimed on the board, scripted in `docs/workflows/owner-list-465-469.workflow.js`. |
| HEAD CHECKED 2026-09-05 NIGHT, SUPERSEDED | `6a64d3eb` = **Rounds 441 to 458**, all merged and gated, code live at douknowball.com (452 was verification only, no code). The four that landed last: **450** 195 two source verified summer 2026 moves applied to the market value table (241 in the overlay, eight left out as unverified by name, the Club Manager rosters re-baked); **456** Rebuild's loop rebuilt to his spec and six real coaches the game had been rating and joking about replaced by generated managers; **457** four private Conquest map renderers collapsed into one shared one with a real takeover animation and measured colour separation on 100k borders; **458** the Stock Market in his format, start seasons 2015 to 2022 measured for depth, cards with numbers only, boot down from 40 requests and 7 MB to 1 request and 24 KiB. Since the 449 row: **451** proved both Club Manager era P1s were fixed by Round 312 and fenced them on his exact 2005/06 Barcelona save (55 checkpoints, 2760 table rows, two controls reproducing the old symptoms); **453** put a flag beside every printed nationality (17 sites, 67 print sites measured across 35 files) and grouped the two long country pickers by confederation with two sources per member list, zero new line wraps at 390 wide; **454** made Sign the Player's "if zero" case real (rivals pass on a lot by tier; 4.3 percent of lots fall when you sit out, held in a 2 to 20 percent band); **455** made World XI's season report say more (gap or margin, longest unbeaten run, player of the season) with 400 of 400 seasons byte identical to before. **441** Sign the Player runs the auction he described: two thirds of auctions could not be finished (a rival start keyed on a flag that never flipped), an uncontested lot sold over its list price, and the end of auction fill put one man in two squads; his 162M Svilar was the pre Round 315 rating curve, already gone, now pinned. **442** Build Your XI stops accepting a goalkeeper outfield (1.58 million player slot decisions, 0 keepers outfield, 0 outfielders in goal, the World XI rule shared through positionFit.ts), the empty search box on USA and South Korea is fixed, and the result screen says something after the AI paragraph. **443** the data sweep he asked for: three games printed numbers the database does not have (Footle kit numbers, Career Ladder assists, Who Am I age and value), 1375 of 1507 pool players have no squad number and now say so; Who Am I boots on 10 requests and 446 KiB. **444** Rarity Round states which way the points run before the first guess, Career Ladder shows a real flag beside every club (116 of 117 club strings), Missing XI bubbles no longer overlap at 320, 390 or 430 wide across every formation (the Round 319 rule had 155 overlapping pairs at 320). **445** Buzzer Beater at /buzzer-beater, ten jump shots on the Free Kick engine lifted into a shared arcade module (Free Kick byte identical after the lift): skilled 5.35x the best fixed release, cap 3012 measured over 800 dates. **446** the report button reaches him and says so (question_reports.emailed, both buttons share one list of kinds; the email leg was proved live and he pasted the proof). **447** the NEW badge means new: 111 of 131 entries carried isNew; every live entry records addedOn from git and the badge is derived for 14 days. **448** the black screen of text after Google sign in is a cover showing the mark on every page (template, prerenderer and build plugin), and the Google spinner resets on open and on return from the back forward cache. **449** World XI no longer prints invented conduct about a real named footballer (400 seeded seasons, 800 lines naming a player, none beside a conduct verb), and Build Your XI shows the same report. Gate on the 441 to 445 batch: 208 of 208 harnesses green in one run, vitest 163 of 163, eight routes played in a browser; each later round gated alone with the build and all eighteen generated site fences green on the merged main. **Two traps this stretch, both written down: a merge can refuse over a file that differs from HEAD only by CRLF (clear it with git show HEAD:path, after checking for real diffs), and the other lane had twelve files of REAL uncommitted work in the main tree (RulesGate once per route plus showReportQuestion hookups, mtime 2026-09-05 02:38) which this lane left alone and did not commit.** Next free number is 450. |
...
| HEAD CHECKED 2026-09-03 LATE, SUPERSEDED | `337ed504` = **Round 429** (Bing Webmaster and Yandex verification, explicit AI crawler groups in robots.txt), on top of **Round 427** parts two to four (the Player Stock Market's Daily mode starts on every date: dailyPrngSeed can return a negative number, dailyCampaignSeed now normalises it with >>> 0 at its own call site, startYearFor wraps the modulo; part one, which changed the shared hash, was reverted because simDaily caught Sign the Player repeating a board), **Round 426** parts one and two (the CFB dynasty stopped bricking at season five: heisman[0] guarded, and the AI roster refill tops up the positions a team is missing instead of indexing ROSTER_SHAPE by roster size, which drained every quarterback, back and receiver out of the league by season five), and **Round 425** (a lost NHL, NBA or NFL Connections daily no longer claims 4/4). Gate on this head: exact TypeScript at zero, all 188 node harnesses green (one clean run, after an earlier run lost simValidatorCache to a Postgres statement timeout that passed on its own), production build, and the seventeen fences that read the template, the public root and the built site all green after the build. Pushed and published the same night. **The adversarial review of 425 to 427 confirmed the daily fixes and found four real things behind them, none a regression, all claimed on the board as follow up parts:** Baseball Connections' result line still counts the padded list on a loss (BaseballConnections.tsx:196, a pre-existing operator precedence bug), unlimited mode pads the same way in all four Connections hooks, a reload on the CFB recap screen replays the final week and the whole postseason and double counts the season (CfbDynastyBoard.tsx:109), and the CBB engine carries the same refill drain. Neither the CFB nor the stock market fix has a fence yet (scripts/cfbDynastyTest.ts is a .ts file the runner never discovers and stops at four seasons; simStockCampaign only walks positive seeds), also claimed. **Round 428 is claimed and in progress: the twelve dailies that reset on refresh**, see the board. Owner asks on 2026-09-03: the games themselves, not infrastructure (his words: he has not seen game work land), Bing and Yandex verified, the spec and the 2026-08-28 tweaks list restated in full (both already in docs, byte identical). |
| HEAD CHECKED 2026-09-02, SUPERSEDED | Round 417 is complete in this head. It hardens the three live schema and freshness fences against thrown transport failures without ever retrying or hiding an HTTP refusal, makes the daily existence probe read zero rows, and keeps Hall of Champions from accepting a partial live catalog. Exact TypeScript passed, all 185 node harnesses passed, the production build passed, and all 15 generated-site fences passed. The prior `origin/main` head was `9542972d`, the Round 416 desktop claim, with Round 415 the last completed round there. Round 416 remains actively claimed and had not landed at this check. Search Console accepted the sitemap on 2026-09-02 with status Success and 137 discovered pages, and all ten priority URLs entered its crawl queue. AdSense shows Getting ready and Review requested. Google's approval decision is pending. The long Round 362 and Round 257 rows below are preserved as history, not current AdSense guidance. |
...
26. That was the Round 289 state. Round 400 closed it by moving every caller to the real
responsive Display slot `7540487748`.
...
- The four restored retired My Career boards can repay their legacy score on a repeat visit.
  ...
  controls that reproduce the leak. No historical points were subtracted.
- A losing NBA Conquest away attacker is annihilated. Round 509 changes that outcome to a
  retreat while preserving territory and shield, with separate rule and result-copy controls.
- `playGames` reports a false stall on `/college-grid` because its answer field says
  `Search for a player`. Round 509 classifies the field by behavior and reaches 14 interactions
  with zero findings. The page itself was healthy.
...
- **Transfer Path, every rule: season keys mix calendar and split styles at 17 clubs**
  ...
  Atlético Mineiro, Vissel Kobe). expandCompactCareers writes "2019" for a calendar spell and
  "2019-2020" for the rest, the graph links only on an identical club::season string, so two
  real teammates at those clubs never link and the refusal says they were never at the same
...
- **Missing XI, "club at the time" convention for a summer tournament.** Nacho's Euro 2024
  ...
  club on the day) and state it in the file header; the hint the page builds prints it.
- **Who Am I's harness claims "no impostor reaches 100" as a measurement**, and it is the
...
everyday rule), chain Griezmann, Koke, Diego Costa, Oblak, Julian Alvarez, and the refused
name Romelu Lukaku after Diego Costa. The refusal is right: Costa was at Chelsea from
2014-15 to 2016-17 and Lukaku in 2011-12 and again in 2021-22, so they were never there in
...
**One money decision, 2026-09-05, from his login screenshots. It is not code.**

1. **Apple sign in.** The code path already exists and is switched off
...
scoreboard header endpoint with no feed account or key.*
...
## Historical roadmap from Round 229

This section preserves the Round 229 audit and its decisions. It is not the
current work order. Use `docs/WORKBOARD.md` for current claims and priorities.

**AUDITED AND REWRITTEN IN ROUND 229** (the old four-item list predated a
hundred rounds and three of its four items had quietly shipped; verified
against the live code, not memory):
...
**Items that were open or resolved at that time:**
...
4. ~~GA4 wiring~~ **DONE, Round 285:** this was originally parked pending a
   measurement ID only Anthony could create. He later supplied `G-KZQK2G68YC`.
   GA4 is live behind cookie acceptance; Lovable analytics remains the source
   for total traffic because it counts visitors who do not accept analytics.
...
| GA4 | `G-KZQK2G68YC`, live since Round 285 and loaded only after cookie Accept. |
| Live scores feed | ESPN's open scoreboard header endpoint since Round 311, with no feed account or key. The `scores-poll` secret still lives only in `private.app_secrets`. The suspended API-Sports account is retired. |
...
- **2026-09-07, Round 509 (Codex lane), COMPLETE, MERGED THROUGH PR 60 AND VERIFIED LIVE.**
  ...
  **THE FOUR MY CAREER REPEAT-CREDIT LEAKS.** NFL, NBA, MLB and NHL restored a retired save
  ...
  could therefore pay the legacy retirement score again on every visit. Round 509 marks the
  restored finish before rendering the retired phase. The component test is 8 of 8 and the
  control exposes positive-score leaks on all four boards. The measured history is 61,964
  points across 30 accounts. No retroactive subtraction was made.

  **NBA CONQUEST.** A failed away attack used the same elimination path as a defeated home
  defender, deleting the attacker and its territory. A losing attacker now retreats, keeps its
  territory and shield, and yields the turn. A successful attacker still eliminates the home
  defender. Separate controls break the hook and make the result copy lie.

  **POLLS AND PUBLIC COUNTS.** Every Poll of the Day is now exactly one named participant
  versus one named participant with choices A and B only. Team rows ask exactly `Who you got?`;
  player rows ask exactly `Who ranks higher all time?`. The fallback carries 33 matchups, the
  component normalizes old rows safely, and the live `poll_head_to_head` plus
  `unique_scheduled_poll_matchups` migrations leave 58 upcoming rows with zero C or D choices,
  zero off-format prompts and zero repeated unordered matchups. Four independent controls cover
  multiway rows, database wording, awkward phrasing and an exact marketing count. The registry
  still knows there are 121 games, while public marketing and the 404 page say `120+`.

  **CAREER DATA AND TRANSFER PATH.** The review expanded from seven conspicuous false club rows
  to the complete 77-row generated 2025-2026 seed and fallback tranche. Every one was authored
  before the season finished, so none is treated as completed-season fact. The fallback removes
  all 77 and the evidence ledger records 73 exact rows plus four variants; 27 separately
  researched 2025-2026 rows stay. Both exact database quarantines are live. Removing the
  projections made 17 Jonathan David puzzles unreachable, so the live companion migration
  deletes those exact rows and refreshes all six derived hint fields for the 885 retained
  puzzles. The replacement verifies 78 active identities by normalized name plus nationality,
  rejects raw-name collisions across the graph, and proves 203 retained puzzles have a valid
  Active Players path. Its exact write-once restore migration is ready and controlled, but stays
  unapplied until the matching frontend is live.

  **THE TESTER, THE DRILLS AND THE CLOCK.** College Grid itself was healthy. `playGames` skipped
  its real answer input because the label contained `search`; behavior-based input classification
  now reaches 14 interactions with zero findings, and the control reproduces the exact stall.
  Soccer Career's wall now reads as connected without changing its legal gap, and the harness
  replays the real daily draws over 90 dates and 900 rounds with zero unwinnable walls plus two
  ...
  **ADSENSE RE-AUDIT.** Seventeen AdSense, legal, SEO, crawler, content and brand checks passed;
  ...
  documents carried publisher verification; all 75 `AdBanner` callers use slot `7540487748`;
  and representative no-JavaScript and browser checks found zero accidental noindex, orphan or
  page error. The one nonblocking follow-up is Club Manager's nearest mobile control at 104px
  from its ad against Google's 150px game-page recommendation, assigned to the lane that owns
  those files. The site-side case is ready. Google still decides and approval is not guaranteed.

  **GOOGLE SIGN IN AND REPORT PRIVACY.** The login screenshot exposed both the raw Supabase
  project host and a personal support email. The paid custom-domain add on is not needed. Google
  now uses its official rendered popup button on douknowball.com, returns an ID token directly to
  ...
  script until Google Branding changes its public support and developer email to
  douknowball1@gmail.com and publishes the DoUKnowBall brand. Email sign in stays available.
...
- **2026-09-02, Round 400 LIVE AND REVIEW SUBMITTED. ADS LOAD ONLY WHERE A PAGE
  ...
  stored accepted consent, the global loader in `index.html` ran on every route,
  including six noindexed snapshots and the bare fallback. Three retired games
  also still rendered a manual slot. The correction keeps the
  publisher verification meta global, removes the global ad loader, and allows
  `loadAdSense` only beside a deliberate slot in a document without noindex.
  The retired slots are removed. Same-tab and cross-tab acceptance reach
  `AdBanner` without reloading an active game, while withdrawal still reloads
  to remove vendor code. Valid games retry briefly while a departing Helmet
  noindex settles; a persistent noindex still blocks the loader. Returning
  accepted visitors read consent on the first render, and the responsive slot
  fills the available width instead of measuring zero.
  **Final local proof:** the exact TypeScript gate and `build:seo` passed; 142
  routes prerendered with zero failures and three account-only routes correctly
  refused; 137 sitemap documents ship with zero noindex mistakes. All 13
  built-site fences passed. simAdsense passed normally and through 14 negative
  controls, simLegalPages through four, simBrand through two. The browser pass
  measured the slot at 358 pixels on mobile and 864 on desktop, found zero ad
  requests on 16 policy-risk routes, passed five lifecycle transitions, and all
  five browser controls reproduced only their owned failures.
  Round 400 merged on `origin/main` as `6cac92c4`, was published live, and
  passed the same route-boundary checks plus a fresh 29-route Google-render
  audit.
  **Account and search actions:** Round 400 created the responsive Display unit
  `DoUKnowBall Game Banner`, slot `7540487748`, and replaced all 75 source
  callers across 75 files, with zero mismatches by scan. Auto Ads and Auto
  optimize were saved OFF and verified false after reopening the site settings.
  Search Console accepted and read the 137-page sitemap on 2026-09-02 with
  status Success, and all ten priority URLs were added to its crawl queue. The
  AdSense review was then submitted. AdSense shows Getting ready and Review
  requested; Google's decision is pending. Detailed evidence is maintained in
...
- **2026-09-02, Round 417. LIVE DATA FENCES SURVIVE RUNNER PRESSURE.** Three
  harnesses that ask the live database about schema or freshness now share one
  three-attempt helper. It retries thrown transport failures after 200ms and
  400ms, then fails; any returned HTTP answer stays immediate for the caller to
  judge, and refusals fail closed. `simDailyPoolOrder` asks for the ordered
  column with `limit=0`, so it proves existence without sorting or scanning the
  table. Its one-throw and HTTP 403 controls prove recovery and refusal paths.
  The first 185-harness gate found a separate live-read flake in
  `simHallOfChampions`: 5 of 10 measured runs lost one or two of its ten
...
- **2026-09-02, Round 416. THE FRONT OFFICE ROSTER GETS A DEFENCE, PART ONE: THE BAKE.**
  ...
  halves were true. `src/data/frontOfficePlayers.ts` held 287 men, all of them QB, RB,
  WR, TE or OL, so no defender could exist to be traded, and its own header says it was
  baked on 2026-08-05 by a script that is not in this repo, so nobody could refresh it.
  It is derived now, by `scripts/genFrontOfficeRoster.mjs`: 480 players over 32 teams,
  an even 15 per club (QB 1, RB 2, WR 3, TE 1, OL 2, DL 2, LB 2, DB 2), including **192
  defenders where there were none**. The trade screen and the roster screen both list
  them, so the literal complaint is answered. The engine still models defence as one team
  number, which is Round 418.
  **Where the data comes from, and why not from here.** The site's own tables cannot do
  it: `nflfastr_player_stats` is offense only and ends at 2024, and the roster table
  ends at 2025. The nflverse rosters release publishes 2026 (2,902 rows, Aaron Rodgers
  reads PIT in it) and its `stats_player` release publishes a 2025 file carrying
  def_sacks, def_tackles_for_loss, def_qb_hits, def_interceptions, def_pass_defended,
  def_fumbles_forced and tackles. Both cache under `scripts/.cache/nflverse/`, which is
  gitignored, and `--check` re-derives and compares.
  **Two source verified.** Every shipped player was checked against ESPN's own public
  2026 rosters: 457 of the 465 ESPN answers for are confirmed on the same team, 98.3
  percent, including Myles Garrett on the Rams, Micah Parsons in Green Bay, Sauce Gardner
  in Indianapolis and Kyler Murray in Minnesota. ESPN returns a 404 for Arizona, which is
  the 15 unchecked. The 8 unconfirmed are not contradicted either: a sweep of the 31
  rosters ESPN does answer for, 2,422 men, places none of them on a different team.
  **HOW A DEFENDER IS RATED, AND THE FIVE TIMES THIS ROUND GOT IT WRONG.** This is the
  whole round. Every failure was found by measuring the output, and the last three were
  found by adversarial review AFTER the round looked finished.
  (1) Ranking the league and keeping each team's best gave a file whose worst starting
  quarterback was 84. Fixed by selecting the squad first, then ranking it against itself.
  (2) Ranking every defender on production put Sauce Gardner, the fourth pick of his
  draft, last among all 64 defensive backs on 28 tackles. The public release carries **no
  coverage column of any kind**, so counting stats rate a corner by how often teams were
  willing to throw at him, which is upside down. Fixed by blending in draft pedigree.
  (3) **An even blend could not have worked, and the arithmetic says so.** Both halves
  are midrank percentiles and therefore uniform on 0 to 1 by construction, so a 50/50
  split cannot carry a bottom decile producer past the MIDDLE of the scale however high
  he was drafted. Pat Surtain II came out at production 0.161 and pedigree 0.839, exactly
  0.500, and shipped at 80, ten below an undrafted corner on 90. Moving Gardner off last
  place had hidden that the same defect sat one rung up.
  (4) **The draft curve never reached the draft.** It was `max(30, 100 - log2(pick)*14)`
  and log2(32)*14 is exactly 70, so it hit the floor at pick 32 and stayed there: 288 of
  the 480 shipped men scored the same on draft as going undrafted. The coefficient comes
  from the draft's own length now, 8.5, so pick 32 scores 57.5 and pick 250 scores 32.3,
  above the undrafted 30.
  (5) **And then the fix for (3) over-corrected, which the second review caught.** At a
  0.75 pedigree weight the corners became a seniority list: Cooper DeJean, a top decile
  producer, shipped at 73 UNDER Rock Ya-Sin, a bottom decile producer, at 83. The real
  driver was not the draft at all. Those two were taken six picks apart, so 15 of the 17
  points between their pedigrees came from years of service, which was worth 3 a year to
  a maximum of 24 against a draft range of 68. Years in the league is not a claim about
  how good anybody is, and it punished exactly the young starters the blend exists to
  treat fairly. Service is worth 1.5 a year to a maximum of 12 now, and the weights were
  swept against named cases rather than chosen: **CB 0.6, S 0.55, LB 0.45, DL 0.35**, a
  ladder from "the numbers cannot see the job" to "the numbers ARE the job", since a sack
  is an event and coverage is not. At 0.5 Derek Stingley Jr. is not even selected; at
  0.75 the producers invert. At 0.6 every named case lands right: Surtain 85, Gardner 89,
  Stingley 92, DeJean 82, all above a defensive back median of 81, with the journeymen
  below it. Garrett 95, Parsons 93, Watt 92, Warner 82, Mahomes 95, Allen 97.
  Two more rules, both measured: production is counted PER GAME above a four game floor,
  because a season total measures availability (Fred Warner played six games in 2025);
  and ties share a midrank, because breaking them on the name handed two men with
  identical numbers the two ends of the scale.
  **The blend decides who is picked as well as what he is worth**, since selection
  ordered defenders on raw production and cut a shutdown corner before the blend saw him.
  Same bug as rating him last, only quieter: a player who is not in the file cannot look
  wrong.
  **THE BAKE NOW FAILS CLOSED.** `num()` returns 0 for a field that is not there, so a
  renamed `games` column routed every player to the pedigree branch while the run still
  printed 32 teams and 480 players and exited 0. Measured by renaming it: 137 of the 480
  names changed and Josh Allen fell from 97 to 78, silently. That is accept on error, in
  the script that decides what 480 real people are worth. It refuses on TWO floors, and it needed
  both: the first is the join itself, at least a third of roster rows finding a stats row
  (the healthy releases manage 1,379 of 1,874, 73.6 percent); the second is that at least
  a third of the matched rows clear the four game bar (1,180 of 1,379 do). The second
  floor is not belt and braces. Guarding the join alone did NOT catch the very failure
  the guard was written for, because a dropped `games` column leaves every row matching
  perfectly and simply reads them all as zero games. The fence caught that gap the moment
  the guard was tightened. The bake prints both numbers on every run now, because a
  silent join is how the whole league got rated on draft position once already.
  **What this method still cannot do, said plainly rather than hidden.** Offensive linemen
  are rated on pedigree alone, because no public feed carries a countable event for them,
  and pedigree cannot see a man drafted in the third round who then became one of the best
  in the league. It gets the top right (Lane Johnson, Quenton Nelson, Andrew Thomas and
  Jake Matthews all 90, Penei Sewell 89) and it gets Creed Humphrey wrong, 80 and 62nd of
  64, because he was pick 63. The damage is bounded on purpose: the linemen's scale is 80
  to 90, a third of the range a defender gets, so the file says "a starting NFL lineman"
  about all of them rather than ranking them confidently. Fence section 8 fails if anyone
  widens that band without first giving linemen something real to be rated on.
  **Two consequences of a bigger roster, both measured.** 15 contracts instead of 9 takes
  mean cap used from 64.8M to 127.0M against the 260M cap, nobody over it and the worst
  placed still holding 65M of room, so free agency and salary matched trades still work.
  And an existing save is left alone on purpose: it carries the whole league object and
  nothing migrates it, so a player mid dynasty keeps the squad he took over, the same rule
  era saves get. He sees the new rosters when he starts a new franchise.
  **Three engine changes, and only the first one changes nothing.** `teamStrength`
  counted every non quarterback, non lineman as a skill player, which with defenders on
  the roster would have counted the defence twice and quietly retuned a sim nobody asked
  to retune; it names the three skill positions now. `replenishRosters` guaranteed a
  quarterback and two linemen and then filled to nine off a cycle of WR, RB, TE, WR, OL,
  so every defender released, traded or retired came back as a receiver and a long save
  drifted back to the offence-only roster this round exists to end; the floor and the
  ...
  **What the harnesses caught, and what two adversarial reviews caught that they did
  ...
  the roster, so Redmond left the bank. `runAllSims` marked the new fence EMPTY, and was
  right to: it printed two lines, and what a harness prints is the only evidence it ran,
  so it reports per section now. The fence itself carries **117 checks and twelve
  controls**, each proven to fail on its own section and only there: `offenceonly`,
  `flat`, `production`, `seasontotals`, `staleheader`, `staleseason`,
  `flatpedigree`, `evenblend`, `openjoin`, `straybucket`, `offencecycle` and
  `doublecount`. Section 9 bundles the real engine with esbuild and runs ten offseasons
  after stripping every defender, so the drain is a behaviour test; `doublecount` bundles
  a deliberately degraded copy of the engine to prove the double count check discriminates.
  **Five fence defects worth recording, because they are the general lesson.** Its parser
  silently dropped the twelve players whose names carry an apostrophe and reported teams a
  man short, so a harness that miscounts blamed the data. Section 7 checked that any year
  in the copy was ROSTER_SEASON or STATS_SEASON, which sounds strict and is not, because
  STATS_SEASON is ALWAYS last year's ROSTER_SEASON: restoring the exact sentence that
  shipped, "Real 2025 rosters" over a 2026 squad, left the fence green on the one bug the
  section exists to stop. Its control substituted 2015, proving only that an implausible
  year is visible. Deleting the years altogether made checks vanish rather than fail, so
  the fence went from 112 to 110 and still exited 0. And Round 416's own teamStrength fix
  had no coverage anywhere in the suite: putting the old filter back left every harness
  green while it moved the number on all 32 teams. **The pattern in all five: a check
  shaped around the offender somebody already found is a check that finds nothing new.**
  Gates: tsc zero, 117 checks with twelve controls, the full node suite, all 15 built site
  fences, build green. Exactly one sitemap row moved date, /front-office. Three snapshots
  this round did not touch (/nba-chain, /nfl-connect-4, /nhl-connect-4) gained a line
  naming their board: that board is picked with `Math.random`, which Round 284
  deliberately freezes with a fixed seed rather than dropping, so it is not a claim about
  today and none of their dates moved. It surfaces because the draw ORDER shifts with a
  new bundle, which is the one residual edge of that mechanism: across the three builds
  this round took, the set of affected pages moved twice before settling. Three flakes, all recorded as flakes and all green on replay: `simPrerenderBoot`
  timed out on /whats-new while five review agents saturated the machine, and
  `simHallOfChampions` and `simWorldXiPositions` both reported SUPABASE UNREACHABLE OR
  POOL TOO SMALL in the final suite run. Neither reads anything this round changed, and
  the second is explained: the Codex lane's Round 417, which this round rebased onto,
  fixed exactly that class of flake, including a Hall of Champions wing lost when one of
  ten concurrent table reads dropped. Both are green on the merged tree.
...
- **2026-08-30, the AdSense verdict, and an audit that found nothing to fix.**
  ...
  **Superseded as a current work order on 2026-09-02, not erased as history:**
  the owner put AdSense readiness first, and Round 400 found a route-level ad
  delivery defect outside this content audit's scope. The owner later authorized
  re-application after live verification, those checks passed, and the review
  was submitted on 2026-09-02. Google's decision remains pending.
...
- **2026-08-29, Round 344.** The stale values sweep. Eight parallel researchers
  ...
  fixed blind; 3 unknowns recorded; 3 verified active names were excluded
  because no value could be sourced. The reviewer gate that needed fixing was
  ...
  committed audit with VALUE_CONTROL=phantom proven red. Round 417's live
  recount remains 145 and matches that ledger exactly: 135 retired, 4
  collisions, 3 unknowns and 3 exclusions.
...
- **2026-08-27, Round 304.** The compliance pass, off the owner's direct instruction ("make sure
  ...
  time activation click happened so reports actually reach the inbox. The CMP wording in this
  Round 304 entry is historical and superseded: Round 400 verified two active European
  regulation messages in Google CMP.
