...
| Shared data layer and provenance | 58% | Round 422 is still in progress and not live: its Player Bingo audit carries 73 exact date-of-birth identities backed by a committed two-source receipt, excludes deceased Diogo Jota, and uses conservative age bounds where an exact birth date is not audited. The Lucas Herrington report is verified but has not changed any data; the next data slice is a current-player identity layer that does not invent a market value or rewrite the historical World Cup snapshot. |
...
| Indexing and SEO | 45% | Round 422 is still in progress and not live. Its working tree keeps the complete crawler copy while preventing it from flashing before React, uses a fresh count-free social-card URL, and rebuilds all route snapshots and the derived sitemap ledger. |
| Profiles and leaderboard | 45% | Round 422 is still in progress and not live. Its working tree separates guest and account streak state, makes hydration failure safe, orders same-page account writes, and prevents a delayed completion from one account mutating another. The backend still keys public history to a mutable display name and saves several tables non-atomically, so stable identity plus one atomic completion RPC is the next P1. Historical point repair remains an owner decision, now 596,072 points across 152 accounts. |
...
Player Bingo's current-age source receipt is
`docs/data/player-bingo-age-audit-2026-09-02.json`. It records the exact
identity, date of birth and two independent authoritative sources for every
audited boundary identity, plus the exclusion evidence for Diogo Jota.
...
| | |
|---|---|
| **CURRENT STATE, CHECKED 2026-09-03. THIS SUPERSEDES THE OLD HEAD ROWS BELOW.** | `origin/main` is `c032df89`. Rounds 419 and 420 have landed there; its tip is the desktop lane's separate Round 421 claim, not a completed Round 421 implementation. Round 422 is on branch `codex/round422-front-door-account-reliability`, whose committed working head is `a2ba5ad5` with final account fixes still in the working tree. Round 422 is not on `main`, is not published, and is not live. Its scope is the front-door crawler-copy paint, count-free social card, account isolation and completion ordering, Player Bingo age audit, Stock Market and Transfer Path report repairs, deterministic first render, six retired Boot Room tombstones, and Stadium Tycoon's prerender and clock fixes. Focused checks and a production SEO build have run during development, but the final full `runAllSims` release pass is still pending, so no final gate or release claim is made here. Round 411 remains claimed separately. Search Console accepted the sitemap on 2026-09-02 with status Success and 137 discovered pages, and all ten priority URLs entered its crawl queue. AdSense shows Getting ready and Review requested. Google's approval decision is pending. The long Round 362 and Round 257 rows below are preserved as history, not current AdSense guidance. |
...
- **2026-09-03, Round 422 IN PROGRESS. THE FRONT DOOR AND ACCOUNT STATE TELL THE TRUTH.**
  The readable crawler copy still ships in every page, but a JavaScript visitor
  no longer sees it flash before React mounts. The no-JavaScript fallback stays
  visible, failed boot recovery restores it, and the root plus every route now
  reference the newly keyed, count-free social card generated by the normal logo
  pipeline. Deterministic loading states replace first-render randomness across
  the reachable app, and six obsolete Boot Room functions return authenticated
  410 tombstones instead of leaving old back ends alive.
  Account hydration now has explicit pending, ready and failed states and two
  separate ordering guards. An auth generation changes whenever the identity
  changes, so no result from account A can become account B state. A monotonic
  request number orders reads inside one generation, so a later same-account
  refresh owns the visible profile and display-name cache. An older initial
  hydration may use only a strictly newer snapshot from that same generation;
  without one it fails instead of replaying its stale row over newer state.
  The conditional default-name claim also re-reads the profile when another
  edit wins.
  Guest, account A and account B keep separate streak, display-name and local
  today state. Storage transitions clear shared slots first. Recovery restores
  a shared display name only after the destination identity marker was written
  and read back successfully, so a blocked marker write cannot expose another
  account's name. A completion captures its auth identity and hydration
  generation when play finishes. Signed-in public `game_completions` rows fail
  closed: they wait for successful same-account hydration, or after a failed
  hydration for a later same-account profile read that verifies the scoped
  public name. They never fall back to the guest handle. Local play still
  counts when that verification is unavailable, without overwriting the remote
  profile streak. Sign-out restores the guest.
  Signed-in completion saves are queued in invocation order so two finishes in
  the same loaded page cannot land an older streak snapshot last. That queue is
  module memory only: it does not order another tab, device or client. The
  backend also still attributes public rows by mutable `player_name` and writes
  `user_game_scores`, `daily_completions`, `user_scores`, `user_best_scores` and
  the profile streak across separate non-atomic requests. The next P1 is a
  stable completion identity and one server-side atomic completion RPC, with
  RLS, idempotency, backfill and negative-control checks before any leaderboard
  repair. The account invitation says plainly that an account saves streaks and
  enables leaderboard play.
  Player Bingo carries 73 exact date-of-birth identities, each backed by the
  committed two-source receipt at
  `docs/data/player-bingo-age-audit-2026-09-02.json`. Unverified identities use
  conservative age bounds rather than a synthetic age, age categories are
  explicit booleans, seed order is stable, and deceased Diogo Jota is excluded.
  Player Stock Market's Retry action now performs a real refetch and its failure
  copy does not invent a cause. Transfer Path clears stale rejection context
  after a valid move and on every puzzle transition.
  Stadium Tycoon no longer starts a live animation-frame loop while prerendering.
  Its save-compatible `matchElapsedSec` accumulator also lets repeated 0.2-second
  ticks advance the match clock instead of leaving live matches at minute zero.
  `simIndexing` now excludes test and spec files from route discovery, removing
  four false page findings without weakening shipped-page coverage.
  A new report said real players are missing and named Lucas Herrington. The
  investigation verified the Australian centre-back's current Hull City and
  Premier League identity from Football Australia plus independent Colorado
  Rapids, Opta, AFC and ABC records, but this round changes no player data. His
  existing 2026 `world_cup_players` row is a historical tournament snapshot
  and stays Colorado Rapids, squad number 25, 4 caps and 0 goals. Current
  valuation sites disagree materially, so no market value is invented and no
  incomplete career row is added. The measured issue is systemic: 343 of the
  1,236 verified 2026 World Cup squad names are absent from every market-value
  year. The planned fix is a separately sourced current-player identity table,
  private field-level source receipts and a derived search view, initially
  published only for Lucas and used only by identity-based autocomplete. The
  exact migration, consumer boundary and controls are recorded in
  `docs/WORKBOARD.md`.
  Verified gates recorded before the branch-wide final runner: exact TypeScript
  zero, full SEO production build with 142 public snapshots and 137 sitemap URLs,
  all 15 generated-site fences, focused Vitest coverage for account races,
  Stock Market, Transfer Path and Stadium Tycoon, the complete Player Bingo age
  and pool controls, render-random and retired-endpoint controls, first-paint and
  soft-404 browser checks, and the normal plus negative-control snapshot drift
  passes. These are interim development checks, not final release gates. The
  full `runAllSims` result is deliberately not claimed here until its final
  release run completes, and the round is neither on `main` nor live.
...
- **2026-09-02, Round 418. THE FRONT OFFICE ENGINE READS THE DEFENCE, PART TWO.** The
  ...
  **The fence flipped, on purpose.** Round 416 asserted that a defender must NOT move
  ...
  a better defender raises it, an old save with no defenders reads the uniform
  replacement-level floor until replenishment, a draft class contains defenders at real
  positions, every prospect becomes a player, a legacy 'DEF' prospect converts, and the
  opening market carries all three groups. The `doublecount` control became `unitdefence`, which bundles the pre 418
