...
| AdSense recovery | 90% | Round 400 passed every local source, build, policy and browser gate. Deployment, live verification, sitemap and ten priority recrawl requests, then the authorized review submission remain. |
...
| | |
|---|---|
| **CURRENT HEAD, CHECKED 2026-09-02. THIS SUPERSEDES THE OLD HEAD ROWS BELOW.** | Round 400 is rebased on `origin/main` at `d49469ae`. Numbered work is landed through the three Round 407 validator addenda. On the final combined tree, Round 400 passed the exact TypeScript gate, the full SEO build, all 13 generated-site fences, the focused AdSense, legal and brand matrix, and the browser lifecycle matrix with all five controls. The final browser pass also caught and fixed a saved private page leaving noindex behind after an in-app move to a public game; the full pass and its exact negative control are green after the fix. Round 400 is not yet on `main` or deployed. The long Round 362 and Round 257 rows below are preserved as history, not current AdSense guidance. |
...
| AdSense | `pub-2929318086316376`. One responsive Display unit exists: `DoUKnowBall Game Banner`, slot `7540487748`. Auto Ads and Auto optimize are verified OFF. Round 400 passed every local gate, is not deployed, and no review was submitted yet. The owner authorized submission after live verification. Detailed evidence lives in `docs/adsense/reapply-readiness.md`. |
| ads.txt line | `google.com, pub-2929318086316376, DIRECT, f08c47fec0942fa0` |
| Google consoles | `anacatu2025@gmail.com`, inspected and configured on 2026-09-02. Current evidence is recorded once in `docs/adsense/reapply-readiness.md`. |
| Search Console | Current sitemap, indexing, inspection, Manual Actions and Security Issues evidence is recorded in `docs/adsense/reapply-readiness.md`. No indexing request has been submitted; the named batch waits until Round 400 is live and verified. |
...
- **2026-09-02, Round 400 LOCAL GATES COMPLETE. ADS LOAD ONLY WHERE A PAGE DELIBERATELY
  OWNS A SLOT.** The August 30 content audit found no measured thin-copy defect,
  ...
  **Final local proof:** the exact TypeScript gate and `build:seo` passed; 142
  ...
  Nothing in this entry claims a commit, deployment, live verification or
  re-application yet.
  **Account and search actions:** Round 400 created the responsive Display unit
  ...
  The detailed AdSense and Search Console evidence, including the exact
  unindexed route batch, is maintained in
  `docs/adsense/reapply-readiness.md`. No review or indexing request was
  submitted yet. The owner authorized both after Round 400 is live and verified.
...
- **2026-08-30, the AdSense verdict, and an audit that found nothing to fix.**
  ...
  **Superseded as a current work order on 2026-09-02, not erased as history:**
  ...
  delivery defect outside this content audit's scope. Re-application still
