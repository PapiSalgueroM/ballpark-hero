...
- S-5 The other career and manager games are "so way behind compared to the soccer ones":
  ...
  live pace, graded verdicts, firing as a real fail state); Round 182 pulled the roles into
  NFL and NBA My Career (depth chart, camp battles, bench seasons, FA role re-evaluation).
  Next on S-5: the same depth chart for NHL and MLB (the 182 recipe transfers directly:
  role field + assign/camp functions + stat scaling + board wiring + chip). Then press
  moments for the careers; press and negotiation depth beyond accept/reject for the GM games.
...
| Next free round number | **183** (check the folder first, the 3-hourly build task may have taken it) |
