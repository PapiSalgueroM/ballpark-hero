...
| `origin/main` head | **Round 299** (the scoring audit: fourteen silent games now earn credit, the coverage guard, the guest name pool). Before it, **Round 298** (the cable-style scores-only ticker, item 1 of the tweaks document). Before it, **Round 297** (the first three tweaks-document items: the created-club league bug fixed at all nine call sites, the no sign-up claim reworded honestly everywhere, Your Dailies removed). Before it, **Round 296** (Transfers Higher or Lower deals the current market, the year scoping guard). Before it, **Round 295** (the report queue: the Missing XI Atalanta teamsheet corrected off three user reports, `simMissingXi.mjs` added, six of eight open reports closed; the first round built, merged and published entirely from a claude.ai/code session). Before it, **Round 294** (Transfer Path hints, pushed 2026-08-26 from Anthony's machine through `RUN294.bat`, the desktop pipeline's last round; every round from 258 to 294 is on `main`). Before that, `9f5dfad` = **Round 293**, the entire 258 to 293 backlog pushed 2026-08-26 (36 rounds, Anthony clicked `SHIP154.bat`, a cloud session watched main advance round by round to 293 and verified the count). Lovable sync was file-verified and `deploy_project` called the same day from the cloud session. Anthony's standing instruction from the same day: shipping and publishing happen without him, the build loop moves to cloud sessions, and his computer stops being part of the pipeline. |
