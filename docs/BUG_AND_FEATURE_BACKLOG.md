...
**Game:** Guess CBB Team / Program | **Complexity:** M | **Status:** DONE (2026-06-14) — root cause was that migration 20260309020228 created cbb_programs/cbb_daily/cbb_scores (public-read RLS) but seeded ZERO rows → infinite "Loading programs...". Frontend was hardened (hook tracks loading/ready/error; board shows error+Retry or "No programs available yet" instead of a permanent spinner), AND the 24-program seed (docs/staged-migrations/DRAFT_cbb_programs.sql, fact-checked) has now been applied by Anthony. /guess-cbb-team confirmed loading and playable. (Daily falls back to a date-seeded pick; no cbb_daily seed required.)
...
Use docs/data/soccer-data.json. Build deterministic generator. Categories: club rosters, nationalities (underused), tournament winners. NO two-club crossovers in autopilot (career data not sourced). Append to docs/connections-batch1-categories.txt. **Status:** PARTIAL — NOTE: the "185" base was wrong; the file actually held **155** puzzles, and **25 of those were silently broken** (a player listed in two groups → dropped by the game's isValidPuzzle filter, which requires 16 unique). FIXED all 25 (each: kept the duplicated player in its correct group, swapped the other instance for a verified player that fits the category and isn't already in the puzzle; a couple of fundamentally-overlapping groups — e.g. the bogus "scored 5 goals in a CL match" group — were rebuilt). Then ADDED 20 new nationality-based puzzles (puzzle-156..175) — nationality groups are collision-proof (one nationality per player) and every player's nationality was verified. File is now **175 fully-valid puzzles**. A programmatic validator confirms all 175 are 4 groups × 4 unique.

**UPDATE (2026-06-14):** added 75 more nationality-based puzzles (puzzle-176..250) via a generator using verified per-nationality player pools (18 nations; dual-nationality edge cases excluded so groups stay unambiguous). File is now **250 fully-valid puzzles** — the 250+ target is met. A programmatic validator confirms all 250 are 4 groups × 4 unique with unique ids and zero within-puzzle collisions; tsc + build pass. (Original "300" remains reachable with another generator pass on the same pools.)
...
**UPDATE (2026-06-14):** added 28 more puzzles (bconn-033..060) → **60 valid puzzles** total. This batch is authored entirely from the two structurally collision-proof templates (birthplace and primary position are mutually exclusive, so no player can satisfy two groups in a puzzle — eliminating the semantic-ambiguity risk that franchise/achievement categories carry). Every player's birthplace/position verified; a programmatic check confirms all 60 puzzles are 4×5 with 20 unique and zero collisions. tsc + build pass. (Target "60+" met; further growth is straightforward with the same templates.)
