...
- WORDS MATCH CODE. Every number and claim in a guide, a commit message, a harness header, a code comment or a FAQ must be what the code does. The review of Rounds 459 to 463 (2026-09-05) found a guide promising a top 300 pool over code drawing 400, a comment saying a script derived a colour block a hand wrote, a guide saying partial clubs "sit at the floor" the code never applied, and a refusal telling a player a real footballer had retired when the table was merely behind. Say what the code does and what the data holds, never more.
- A game that records completions under a gameId that is not its route path needs its entry in COMPLETION_SLUG_TO_PATH (src/data/completionSlugs.ts), or the daily chip never ticks and Most Played drops it; scripts/simDailyLegend.mjs is the fence, run it whenever you add a route or a recorder.
- Every new step in a flow (a new screen, a hand over, the next seat's prompt) gets the reveal ref from src/hooks/useRevealScroll.ts so a 390 wide phone sees it without scrolling; measure it in the browser, the review found a prompt 267 pixels above the screen.
- Generated names: every first name and surname bank you add must be registered in scripts/simInventedNames.mjs GENERATORS and no pairing may name a real player (Bruno plus Varela did); run that harness.
- Main is at 0eb85f80 (2026-09-05 evening): Rounds 459 to 464 are on it, Round 462 rewrote the era Champions League and the tiebreaks in src/lib/clubManager.ts, and Round 450 re-baked the rosters, so measure against main, never against a memory of it.
...
2. Gates from the integration worktree, in this order: tsc (zero, READ THE EXIT CODE); every new or changed harness (git diff --name-only main..HEAD -- scripts), each with every negative control, and each control must actually fire; then every scripts/simClubManager*.mjs and simCm*.mjs, simManagers.mjs, simFreeKick.mjs and simBuzzerBeater.mjs with their controls, simCareerMoneyAfterRetirement.mjs, simCareerBanking.mjs, simNoInventedConduct.mjs, simDailyReload.mjs, simCompletionOnce.mjs, simLeaderboardCaps.mjs, simScoringCoverage.mjs, simNoRivalNames.mjs, simDailyLegend.mjs, simCompletionSlugs.mjs, simInventedNames.mjs, simBoardObjectives.mjs, simDealDepth.mjs, simNationalityFlags.mjs, sweepSaves.mjs if it runs headless, and node node_modules/vitest/vitest.mjs run. A control is judged by its last line saying its planted defect fired, never by its exit code: simConquestMap's controls exit non zero by design, and each harness's control env var is named in its own header (read it, do not guess).
