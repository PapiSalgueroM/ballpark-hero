...
| 7 | DONE, Round 566 (2026-09-13): `docs/data/soccer-data.json` tournament_winners | 8 keys, 6 wrong | Six of the eight held a DIFFERENT nation's squad than the key named ("WC 2022 Argentina winners" was Ecuador's, "Euro 2024 Spain winners" was Germany's). Proved a second time independently of the 2026-05-29 audit by resolving every list against `playerNationalities.ts`: each of the eight came back as exactly ONE nationality with no mixture, and the same six disagreed with their key. The two World Cup keys now carry the real winners read from `national_team_squads` (France 2018, Argentina 2022); the other four are re-keyed to the nation they really are and no longer claim "winners", because no table here holds Spain's Euro 2024, Italy's Euro 2020, Argentina's Copa America 2021 or Senegal's AFCON 2021 and inventing them is forbidden. Nothing deleted, 111 real names went from libelled to correct. Fenced by `scripts/simTournamentWinners.mjs` (controls `miskey`, `reader`), which also fails if anything under src, scripts or supabase starts reading the file, because it is a generation input and not a shipped source, and if the quarantined `_DO_NOT_APPLY_` Connections migration stops being quarantined | Done; the original suggestion (rebuild from national_team_squads) was only half possible, that table covers the World Cup and nothing else and world_cup_players |
| 8 | PART, Round 561: `teammatesPairs.ts` (50), `ufcFighters.ts` (112), `ufcChainData.ts` (64) | 226 | The cross check now exists: `scripts/simUfcFacts.mjs` (controls `record`, `crossfile`, `finishes`) compares the two UFC files on all 62 fighters they share and pins what each row can compute about itself, which found four weight class disagreements nothing was comparing. Those turned out not to be errors: the field answers a different question in each file (the chain file's value decides which division's chain a fighter belongs to, and the chain mode filters the graph by it), so the four two division fighters are declared with both divisions and the harness checks the declared pair is the pair observed and that the two are neighbouring divisions. Round 562 then added `scripts/simTeammatesPairs.mjs` (controls `flipfalse`, `wrongclub`, `dupe`), which adjudicates 10 of the 16 soccer rows against `soccer_player_club_stints` and confirms each one's club, checks every funFact agrees in sign with the answer it explains, and refuses duplicate pairings. **Still open: none of the 226 rows is two source verified**, and the 34 NBA and NFL teammate rows cannot be adjudicated at all because `nba_player_team_seasons` is empty. A green run on either harness means consistent, not verified, and both headers say so | Two source the numbers, which needs the web. **Do not use `public.mma_fighter_careers` as the second source**, it looks like the answer and is a half captured scrape: 15 of its 86 rows claim more finishes than wins, 63 of 86 have a prose career history with reference markers where the weight class should be, and it has Conor McGregor at 1-0 and Stipe Miocic at 5-5. Nothing in the site reads it. For the six unadjudicated soccer pairs, the gap is the career table's, not the file's: Lampard has one stint row for four clubs and Pirlo two for three, and neither carries the MLS spell that settles three of them |
